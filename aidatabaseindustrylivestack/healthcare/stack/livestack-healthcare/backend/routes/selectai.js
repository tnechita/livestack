...
function logTrace(trace, status, extra = {}) {
  const payload = {
    event: 'ask_healthcare_data_request',
...
router.get('/schema-objects', async (_req, res) => {
  ...
  res.json({
    objects,
    domains: groupHealthcareSchemaObjectMetadata(objects),
    meta: {
...
router.get('/health', async (req, res) => {
  ...
  const result = await checkAskHealthcareDataHealth({ demoUser: req.demoUser, profile });
