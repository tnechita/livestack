/*
 * load_graph_data.sql
 * Signal-source connections (3000+ edges) and manufacturer-source links
 */

SET SERVEROUTPUT ON
PROMPT Loading signal graph edges...
...
BEGIN
    ...
    -- Generate ~3000 signal source connections. Values use the original
    -- constrained graph edge vocabulary, but represent signal propagation.
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Signal source connections loaded: ' || v_count);

    -- Generate manufacturer-source links (~1500)
    v_count := 0;
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Manufacturer-source links loaded: ' || v_count);
END;
/
