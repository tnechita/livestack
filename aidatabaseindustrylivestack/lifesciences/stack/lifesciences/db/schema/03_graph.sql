/*
 * 03_graph.sql
 * Property Graph for signal-source networks and manufacturer signal propagation
...
-- ============================================================
-- MANUFACTURER ↔ SIGNAL-SOURCE RELATIONSHIPS
-- ============================================================
...
FROM GRAPH_TABLE ( influencer_network
    ...
    WHERE a.handle = '@fda_watch'
      AND b.handle = '@hazmat_ops'
    COLUMNS (
        a.handle AS source,
        b.handle AS target,
...
-- Manufacturer propagation: which signal sources amplify a manufacturer through the network
...
FROM GRAPH_TABLE ( influencer_network
    ...
    WHERE b.brand_name = 'VitaCore Therapeutics'
    COLUMNS (
        v1.handle, v2.handle,
        e1.relationship_type,
        e2.connection_type,
