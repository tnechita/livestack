...
- **Fixture layout**
  - `verification/fixtures/required` mirrors the mandatory tables shipped with every bundle (`brands`, `products`, `fulfillment_centers`, `customers`, `influencers`, `social_posts`, `post_product_mentions`, `inventory`, `orders`, `order_items`). Each CSV uses the exact header order emitted by `backend/lib/importCatalog.js`.
...
- **Checklist**
  ...
  - **Foreign keys:** `orders.customer_id`, `order_items.product_id`, `inventory.center_id`, and other reference columns must resolve to source IDs in the uploaded CSVs; broken references are flagged pre-import.
...
- **Derived and regenerated data**
  ...
  - `product_embeddings`, `post_embeddings`, and `semantic_matches` run after the base import, using the `ALL_MINILM_L12_V2` vector model; if the model is unavailable, the import finishes but warns about the skipped vector refresh.
