...
const generationStore = require('./datasetGenerationStore');
const { beginDatasetServingTransition, associateDatasetServingTransition, endDatasetServingTransition } = require('./datasetServingFence');
const { collectFeatureEvidence, assertRequiredFeatureEvidence } = require('./datasetFeatureEvidence');
...
function calculateViralityScore(row) {
  const momentum = String(row?.momentum_flag || '').toLowerCase();
  ...
  const reachScore = Math.min(18, Math.log10(Math.max(views, 1)) * 3);
  const engagementScore = Math.min(
    12,
    Math.log10(Math.max(likes + (shares * 2) + (comments * 3), 1)) * 2
  );

  return roundTo(Math.min(100, momentumBase + reachScore + engagementScore), 2);
}
...
function buildTemplateReadme() {
  return [
    ...
    '- Domain labels use the original schema names for compatibility: brands are manufacturers, products are life sciences products, influencers are signal sources, and social_posts are regulatory or supply bulletins.',
...
function cloneParsedImportDataset(parsed) {
  const cloned = {
    ...parsed,
  ...
  const tables = {};
  for (const [tableName, tableData] of Object.entries(parsed.dataset.tables || {})) {
    tables[tableName] = {
      ...tableData,
      rows: (tableData.rows || []).map((row) => {
        const clonedRow = {};
        for (const [key, value] of Object.entries(row)) {
          clonedRow[key] = cloneImportValue(value);
        }
        return clonedRow;
      }),
      sourceIds: new Set(tableData.sourceIds || []),
      header: tableData.header ? [...tableData.header] : tableData.header,
    };
  }
...
function getBundledDemoDateAnchor(dataset) {
  const anchorColumns = [
  ...
  let anchor = null;
  for (const [tableName, columnName] of anchorColumns) {
    for (const row of dataset.tables[tableName]?.rows || []) {
      const value = row[columnName];
      if (!(value instanceof Date) || Number.isNaN(value.getTime())) continue;
      if (!anchor || value > anchor) anchor = value;
    }
  }
...
function rebaseBundledDemoDates(parsed, targetDate = new Date()) {
  const cloned = cloneParsedImportDataset(parsed);
  const dataset = cloned.dataset;
  if (!dataset) return cloned;

  const sourceAnchor = getBundledDemoDateAnchor(dataset);
  if (!sourceAnchor) return cloned;

  const timestampOffsetMs = targetDate.getTime() - sourceAnchor.getTime();
  const dayOffset = Math.round((getUtcDayStart(targetDate) - getUtcDayStart(sourceAnchor)) / ONE_DAY_MS);

  for (const tableData of Object.values(dataset.tables)) {
    for (const column of tableData.table.columns) {
      ...
      for (const row of tableData.rows) {
        ...
        if (column.type === 'date') {
          ...
          row[column.name] = shifted;
        } else {
          row[column.name] = new Date(value.getTime() + timestampOffsetMs);
        }
      }
    }
  }
  ...
  dataset.manifest = {
    ...dataset.manifest,
    rebasedAt: targetDate.toISOString(),
    dateAnchor: {
      source: sourceAnchor.toISOString(),
...
function getBundledDemoDataset(version = IMPORT_VERSION) {
  ...
  return {
    archive: cachedBundledDemoDataset.archive,
    parsed: rebaseBundledDemoDates(cachedBundledDemoDataset.parsed),
  };
}
...
function resolveInsertValue(table, column, row, idMaps) {
  const value = row[column.name];
  if (table.name === 'social_posts' && column.name === 'virality_score' && value == null) {
    return calculateViralityScore(row);
  }
...
function buildFallbackShipments(dataset, idMaps) {
  ...
  const carriers = ['CryoLine', 'TrialFreight', 'SafeTemp', 'Clinical Express'];
  const shipStatusMap = {
    confirmed: 'preparing',
    processing: 'packed',
    shipped: 'in_transit',
...
async function applyOptionalFallbacks(connection, dataset, idMaps, warnings, progress) {
  ...
  if (!dataset.tables.demand_regions.provided) {
    ...
    if (!generatedDemandRegions.length) warnings.push('No fallback demand_regions could be generated because trial site geospatial data was missing.');
  }
...
async function executeImportPlan({ dataset, dryRun = false, progress = null }) {
  let connection;
  const warnings = [];
...
async function startDatasetJob({ parsed, kind, lockMessage, queuedMessage, startMessage, completeMessage, datasetSource, initiatingActor }) {
  ...
  let transitionToken;
  try {
    admission = await generationStore.admitGeneration({ jobId: job.jobId, initiatingActor, operation: kind });
    transitionToken = beginDatasetServingTransition({ generationId: admission.generationId, jobId: job.jobId, status: 'admitted' });
  } catch (error) {
    endOperation();
    throw error;
  }
  ...
  setImmediate(async () => {
    try {
      await generationStore.transition({ ...admission, jobId: job.jobId, status: 'applying', message: startMessage });
      associateDatasetServingTransition(transitionToken, { status: 'applying' });
      updateJob(job.jobId, {
        status: 'running',
        progress: 5,
      ...
      // The active Life Sciences generation is published only after its native
      // feature claims have been observed in Oracle and persisted with the job.
      const requiredFeatures = await collectFeatureEvidence();
      requiredFeatures.generationId = admission.generationId;
      assertRequiredFeatureEvidence(requiredFeatures);
      await generationStore.recordFeatureEvidence({
        ...admission,
        jobId: job.jobId,
        evidence: requiredFeatures,
      });
      const activeDataset = await persistDatasetState(datasetSource, warnings);
      await generationStore.completeGeneration({ ...admission, jobId: job.jobId, activeDataset: buildDatasetState(datasetSource) });
      ...
      try { await generationStore.failGeneration({ ...admission, jobId: job.jobId, message: err.message || 'Import failed.' }); } catch (lifecycleError) { console.error('Could not finalize failed Life Sciences dataset generation:', lifecycleError.message || lifecycleError); }
      updateJob(job.jobId, {
        status: 'failed',
        progress: 100,
...
async function generateTemplateArchive({ version = IMPORT_VERSION }) {
  ...
  return {
    buffer: zip.toBuffer(),
    fileName: `Lifesciences-import-template-${version}.zip`,
...
async function startImport({ req, body = {}, demoUser = null, version = IMPORT_VERSION }) {
  const archive = getArchiveBufferFromRequest({ req, body });
  const parsed = parseArchiveDataset(archive.buffer, version);
...
async function startDemoRestore({ demoUser = null, version = IMPORT_VERSION }) {
  const demoDataset = getBundledDemoDataset(version);
  return await startDatasetJob({
    parsed: demoDataset.parsed,
    kind: 'restore_demo',
