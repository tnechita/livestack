...
const { createRequireDatasetAdmin } = require('../lib/datasetAdminAuthorization');
...
const requireDatasetAdmin = createRequireDatasetAdmin({
  resolveDatasetAdminActor: db.resolveDatasetAdminActor,
});
...
router.get('/template', async (req, res) => {
  ...
  try {
    ...
    const fileName = archive.fileName || `Lifesciences-import-template-${version}.zip`;
...
router.post('/upload', requireDatasetAdmin, uploadSingleFile, async (req, res) => {
  const service = getImportService(req);
...
router.post('/restore-demo', requireDatasetAdmin, async (req, res) => {
  const service = getImportService(req);
