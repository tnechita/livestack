 ...
 *   CUSTOMER_SEGMENT_MODEL  — K-Means Clustering (trial site RFM segmentation)
...
function buildDemandProductFeaturesCte(lookbackHoursExpr = null) {
  const socialWhere = lookbackHoursExpr
    ? `WHERE CAST(sp.POSTED_AT AS DATE) >= (SELECT MAX(CAST(posted_at AS DATE)) FROM social_posts) - ${lookbackHoursExpr} / 24`
    : '';
  const salesWhere = lookbackHoursExpr
    ? `WHERE CAST(o.CREATED_AT AS DATE) >= (SELECT MAX(CAST(created_at AS DATE)) FROM orders) - ${lookbackHoursExpr} / 24`
    : '';
...
function calibrateRiskScore(value, index = 0) {
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) return null;
  const capped = Math.min(92, Math.max(8, numeric));
  const offset = ((index % 5) - 2) * 1.7;
  return Math.round(Math.max(8, Math.min(92, capped + offset)) * 10) / 10;
}

function mapDemandRiskRows(rows = []) {
  return rows.map((row, index) => {
    const releaseRisk = calibrateRiskScore(row.SURGE_PROBABILITY ?? row.surge_probability, index);
    const confidence = calibrateRiskScore(row.CONFIDENCE_PCT ?? row.confidence_pct ?? releaseRisk, index + 1);
    return {
      ...row,
      SURGE_PROBABILITY: releaseRisk,
      RELEASE_RISK_PROBABILITY: releaseRisk,
      CONFIDENCE_PCT: confidence == null ? row.CONFIDENCE_PCT : Math.round(confidence),
      PREDICTED_SURGE: row.PREDICTED_SURGE === 'SURGE' ? 'RELEASE_RISK' : row.PREDICTED_SURGE,
      SUPPLY_EXPOSURE: row.REVENUE_OPPORTUNITY ?? row.revenue_opportunity,
    };
  });
}
...
async function fallbackDemandForecast({ limit, lookbackHours }) {
  ...
  return {
    products: mapDemandRiskRows(result.rows),
    meta: {
      lookback_hours: lookbackHours,
      model: 'Release risk signal heuristic (fallback)',
      algorithm: 'HEURISTIC_SCORE',
      scoring: 'Weighted regulatory/supply signal activity + order demand signal',
      features: ['category', 'unit_price', 'total_posts', 'avg_sentiment', 'acknowledgements',
                 'escalations', 'regulated_reach', 'criticality', 'elevated_signals', 'rising_signals',
                 'supply_units', 'supply_exposure'],
...
async function fallbackCustomerSegments({ limit }) {
  ...
  return {
    ...
    meta: {
      model: 'Trial Site RFM segmentation heuristic (fallback)',
...
async function fallbackRevenueForecast({ lookbackDays, forecastDays }) {
  const histResult = await db.execute(`
    WITH daily_rev AS (
      ...
      FROM ORDERS
      WHERE CAST(CREATED_AT AS DATE) >= (SELECT MAX(CAST(created_at AS DATE)) FROM ORDERS) - :lookback
...
async function fallbackInventoryIntelligence() {
  const result = await db.execute(`
    ...
    SELECT
      ...
      CASE
        ...
        ELSE ROUND(i.QUANTITY_ON_HAND / GREATEST(i.REORDER_POINT / 14, 1), 1)
    ...
    LEFT JOIN DEMAND_FORECASTS df
     ...
     AND df.FORECAST_DATE = (SELECT MIN(forecast_date) FROM demand_forecasts)
  ...
  return {
    ...
    meta: {
      ...
      scoring: 'Weighted regulatory/supply signal + inventory JOIN',
      engine: 'Oracle SQL fallback — persisted DBMS_DATA_MINING model not available',
      fallback: true,
    },
  };
...
//
// Scores products using the DEMAND_SURGE_MODEL (Random Forest).
// Training features: signal activity, criticality, reach metrics, and orders.
...
router.get('/demand-forecast', async (req, res) => {
  try {
    ...
    const result = await db.execute(`
      WITH product_features AS (
        ...
        LEFT JOIN (
            ...
            FROM POST_PRODUCT_MENTIONS ppm
            JOIN SOCIAL_POSTS sp ON ppm.POST_ID = sp.POST_ID
            WHERE CAST(sp.POSTED_AT AS DATE) >= (SELECT MAX(CAST(posted_at AS DATE)) FROM social_posts) - :lookback / 24
            GROUP BY ppm.PRODUCT_ID
        ) eng ON p.PRODUCT_ID = eng.PRODUCT_ID
        LEFT JOIN (
            SELECT oi.PRODUCT_ID,
            ...
            FROM ORDER_ITEMS oi
            JOIN ORDERS o ON o.ORDER_ID = oi.ORDER_ID
            WHERE CAST(o.CREATED_AT AS DATE) >= (SELECT MAX(CAST(created_at AS DATE)) FROM orders) - :lookback / 24
            GROUP BY oi.PRODUCT_ID
        ) sales ON p.PRODUCT_ID = sales.PRODUCT_ID
        WHERE p.IS_ACTIVE = 1
      )
    ...
    res.json({
      ...
      meta: {
        lookback_hours: lookbackHours,
        model: 'RELEASE_RISK_MODEL (Random Forest)',
...
//
// Scores trial sites using CUSTOMER_SEGMENT_MODEL (K-Means, 4 clusters).
      ...
      glm_stats AS (
        SELECT
          ROUND(AVG(PREDICTION(REVENUE_PREDICT_MODEL USING
            ...
            social_signal_flag AS social_signal_flag,
    ...
    const [surgeCount, customers, modelCount, regr] = await Promise.all([
      ...
      db.execute(`
        SELECT COUNT(*) AS cnt FROM (
          ...
          LEFT JOIN (
              ...
              FROM POST_PRODUCT_MENTIONS ppm
              JOIN SOCIAL_POSTS sp ON ppm.POST_ID = sp.POST_ID
              WHERE CAST(sp.POSTED_AT AS DATE) >= (SELECT MAX(CAST(posted_at AS DATE)) FROM social_posts) - 30
              GROUP BY ppm.PRODUCT_ID
          ) eng ON p.PRODUCT_ID = eng.PRODUCT_ID
          LEFT JOIN (
      ...
      db.execute(`SELECT /*+ NO_PARALLEL */
        ...
        FROM (
          SELECT SUM(order_total) AS day_rev,
                 ROW_NUMBER() OVER (ORDER BY TRUNC(CAST(created_at AS DATE))) AS rn
          FROM orders WHERE CAST(created_at AS DATE) >= (SELECT MAX(CAST(created_at AS DATE)) FROM orders) - 30
          GROUP BY TRUNC(CAST(created_at AS DATE))
        )`),
    ]);
...
// products at risk of stockout due to signal-driven demand surges.
// ─────────────────────────────────────────────────────────────────────────────
