/**
 * Fulfillment API — Spatial routing and cold-chain site management
 */
...
// GET /api/fulfillment/nearest — find nearest site for a trial site+product
...
// Returns trial site lat/lon + tier for the Trial Site Tier spatial layer
router.get('/customers', async (req, res) => {
  try {
    ...
    console.error('Trial Site layer error:', err);
    res.status(500).json({ error: err.message });
  }
});
      ...
      SELECT r.region_id,
             ...
             (SELECT ROUND(AVG(df.predicted_demand), 0)
              ...
              WHERE UPPER(df.region) = UPPER(r.region_name)
                AND df.forecast_date BETWEEN (SELECT MIN(forecast_date) FROM demand_forecasts)
