/**
 * Signal Feed API — regulatory bulletins, source trends, and vector search
 */
...
// GET /api/social/posts — paginated signal feed
  ...
  try {
    ...
    const result = await db.executeAsUser(`
      SELECT sp.post_id, sp.platform, sp.post_text, sp.posted_at,
             ...
             i.handle AS signal_source_code,
             i.display_name AS influencer_name,
             i.display_name AS signal_source_name,
             i.follower_count,
             i.follower_count AS source_reach,
             i.influence_score,
             i.influence_score AS source_authority_score,
             sp.likes_count AS acknowledgement_count,
             sp.shares_count AS escalation_count,
             sp.comments_count AS case_count,
             sp.views_count AS regulated_reach
...
// GET /api/social/influencers — lightweight list of signal-source handles for dropdown filters
router.get('/influencers', async (req, res) => {
  try {
    ...
    console.error('Signal source list error:', err);
    res.status(500).json({ error: err.message });
  }
});

// GET /api/social/viral — elevated and critical signal records
router.get('/viral', async (req, res) => {
  try {
    ...
    console.error('Critical signals error:', err);
    res.status(500).json({ error: err.message });
  }
});
  ...
  try {
    const result = await db.executeAsUser(`
      SELECT platform,
             COUNT(*) AS post_count,
             SUM(likes_count) AS total_likes,
             SUM(likes_count) AS total_acknowledgements,
             SUM(shares_count) AS total_shares,
             SUM(shares_count) AS total_escalations,
             SUM(views_count) AS total_views,
             SUM(views_count) AS total_regulated_reach,
             ROUND(AVG(sentiment_score), 3) AS avg_sentiment,
             COUNT(CASE WHEN momentum_flag IN ('viral','mega_viral') THEN 1 END) AS critical_signal_count
      FROM social_posts
...
// POST /api/social/post-search — vector similarity search over signal bulletins
  ...
  try {
    ...
    const result = await db.executeAsUser(`
      SELECT sp.post_id, sp.platform, sp.post_text, sp.posted_at,
             sp.likes_count, sp.shares_count, sp.comments_count, sp.views_count,
             sp.sentiment_score, sp.virality_score, sp.momentum_flag,
             i.handle AS influencer_handle, i.handle AS signal_source_code,
             i.display_name AS influencer_name, i.display_name AS signal_source_name,
             i.follower_count, i.follower_count AS source_reach,
             i.influence_score, i.influence_score AS source_authority_score,
             sp.likes_count AS acknowledgement_count,
             sp.shares_count AS escalation_count,
             sp.comments_count AS case_count,
             sp.views_count AS regulated_reach,
