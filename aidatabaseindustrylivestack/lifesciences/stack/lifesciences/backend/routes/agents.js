...
const STATIC_TEAMS = [
  {
    ...
    STATUS: 'ENABLED',
    DESCRIPTION: 'Ollama-backed regulatory and market signal analysis over live life sciences supply data.',
  },
  {
    TEAM_NAME: 'FULFILLMENT_TEAM',
    STATUS: 'ENABLED',
    DESCRIPTION: 'Ollama-backed cold-chain fulfillment analysis using inventory and routing context.',
  },
  {
    TEAM_NAME: 'COMMERCE_TEAM',
    STATUS: 'ENABLED',
    DESCRIPTION: 'Ollama-backed clinical supply order analysis using orders and revenue context.',
  },
];
...
function teamDisplayLabel(teamName) {
  return TEAM_DISPLAY_LABELS[teamName] || teamName;
}
...
async function buildAgentContext(teamName) {
  if (teamName === 'SOCIAL_TREND_TEAM') {
    const [summary, products, influencers, momentum] = await Promise.all([
      ...
      db.execute(
        `SELECT /*+ NO_PARALLEL */ p.product_name, b.brand_name, p.category,
         ...
         WHERE CAST(sp.posted_at AS DATE) >= (SELECT MAX(CAST(posted_at AS DATE)) FROM social_posts) - 2
      ...
      db.execute(
        `SELECT momentum_flag, COUNT(*) AS post_count
         FROM social_posts
         WHERE CAST(posted_at AS DATE) >= (SELECT MAX(CAST(posted_at AS DATE)) FROM social_posts) - 2
    ...
    return {
      instructions: 'Focus on products, manufacturers, regulatory signals, criticality, and concrete metrics.',
      context: {
        team: teamName,
  ...
  if (teamName === 'FULFILLMENT_TEAM') {
    ...
    return {
      instructions: 'Focus on inventory risk, cold-chain routing, and practical fulfillment actions.',
      context: {
        team: teamName,
  ...
  const [summary, categories, orderStatus] = await Promise.all([
    db.execute(
      `SELECT COUNT(*) AS total_orders,
       ...
       FROM orders
       WHERE CAST(created_at AS DATE) >= (SELECT MAX(CAST(created_at AS DATE)) FROM orders) - 30`
    ),
    db.execute(
      `SELECT p.category,
       ...
       WHERE CAST(o.created_at AS DATE) >= (SELECT MAX(CAST(created_at AS DATE)) FROM orders) - 30
       GROUP BY p.category
  ...
  return {
    instructions: 'Focus on clinical supply orders, revenue, signal attribution, and business trends.',
    context: {
      team: teamName,
...
function conversationContext(history = []) {
  return (Array.isArray(history) ? history : [])
    ...
    .slice(-6)
    .map((entry) => `${entry.role === 'assistant' ? 'Assistant' : 'User'}: ${String(entry.text).replace(/\s+/g, ' ').trim().slice(0, 500)}`)
    .join('\n');
}
...
async function askAgent(teamName, question, history = []) {
  ...
  const contextualQuestion = priorConversation
    ? `Prior conversation:\n${priorConversation}\n\nCurrent follow-up: ${question}`
    : question;
  try {
    return await Promise.race([
      summarizeContext({ question: contextualQuestion, instructions, context }),
...
// Runs the signal team to identify critical product signals.
  ...
  try {
    ...
    const productsResult = await db.execute(
      `SELECT /*+ NO_PARALLEL */ p.product_id, p.product_name, b.brand_name,
       ...
       JOIN products p      ON ppm.product_id = p.product_id
       JOIN brands b        ON p.brand_id     = b.brand_id
       WHERE CAST(sp.posted_at AS DATE) >= (SELECT MAX(CAST(posted_at AS DATE)) FROM social_posts) - :hours/24
    ...
    const distResult = await db.execute(
      `SELECT /*+ NO_PARALLEL */ momentum_flag, COUNT(*) AS post_count
       FROM social_posts
       WHERE CAST(posted_at AS DATE) >= (SELECT MAX(CAST(posted_at AS DATE)) FROM social_posts) - :hours/24
       GROUP BY momentum_flag
       ORDER BY post_count DESC`,
      { hours }
    );
    ...
    try {
      agentAnalysis = await Promise.race([
        askAgent('SOCIAL_TREND_TEAM',
          `Identify the top product signals and signal sources from the last ${hours} hours ` +
    ...
    res.json({
      message:      `Signal detection complete - ${products.length} critical product signals identified in last ${hours}h`,
      trending:     trendText,
  ...
  try {
    ...
    try {
      trendAnalysis = await Promise.race([
        askAgent('SOCIAL_TREND_TEAM',
          'What product signals are critical right now based on regulatory and supply activity in the last 48 hours'),
        new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 10000)),
      ]);
    } catch (e) {
    ...
    const commerceResult = await db.execute(
      `SELECT /*+ NO_PARALLEL */
              ...
              ROUND(AVG(order_total), 2) AS avg_order_value
       FROM orders
       WHERE CAST(created_at AS DATE) >= (SELECT MAX(CAST(created_at AS DATE)) FROM orders) - 7`
    );
    ...
    try {
      commerceAnalysis = await Promise.race([
        askAgent('COMMERCE_TEAM',
          'Summarize signal-driven clinical supply orders and revenue attribution from the last 7 days'),
        new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 10000)),
      ]);
    } catch (e) {
    ...
    await logAction('master_orchestrator', 'commerce_attribution', 'orders', null, {
      ...
      reason: `${socialPct}% of orders ($${(commerce.SOCIAL_REVENUE || 0).toLocaleString()}) attributed to compliance and supply signals in last 7 days`,
    }, 0.93);
    ...
    res.json({
      message: `Full cycle complete - ${topProducts.length} signals · ${criticalInventory.length} inventory alerts · ${socialPct}% signal-driven orders`,
      phases: {
        trends: {
...
router.post('/trends', async (req, res) => {
  try {
    ...
    const q = question || 'What product signals are critical right now based on regulatory and supply activity';
...
router.post('/chat', async (req, res) => {
  ...
  const priorConversation = conversationContext(history);
  const qLower = `${priorConversation}\n${q}`.toLowerCase();
  ...
  const trendStrong = ['trending', 'viral', 'virality', 'mega_viral', 'momentum', 'influencer', 'source', 'bulletin', 'regulatory', 'compliance', 'criticality', 'cold-chain notice', 'rising'];
  const inventoryStrong = ['inventory', 'warehouse', 'fulfillment', 'reorder', 'restock', 'out of stock', 'cold-chain', 'allocation'];
  ...
  const commerceWeak = ['order', 'customer', 'trial site', 'price', 'category', 'brand', 'manufacturer', 'product', 'product', 'total'];
  ...
  try {
    if (intent === 'trends') {
      ...
      fallbackResult = trendRes.rows[0]?.RESULT || 'No critical product signals found';
      ...
      // Check if this is a routing question that mentions a trial site or city.
      const cityMatch = q.match(/(?:to|in|near)\s+(?:a\s+(?:customer|trial site)\s+in\s+)?([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)/);
      const emailMatch = q.match(/[\w.]+@[\w.]+/);

      if (productName && (cityMatch || emailMatch)) {
        // Spatial routing: find best fulfillment site.
        let customerEmail = emailMatch ? emailMatch[0] : null;

        // If user gave a city name, look up a real trial site email in that city.
        if (!customerEmail && cityMatch) {
          ...
          try {
            ...
            if (custRes.rows.length > 0) {
              ...
              toolsUsed.push({ tool: 'trial site_lookup', params: { city: cityName }, status: 'success', email: customerEmail });
            }
          } catch (_) {}
        }
      ...
      // clinical supply order and revenue queries.
      ...
      fallbackResult = `Last 30 days: ${(c.TOTAL_ORDERS || 0).toLocaleString()} orders, $${(c.TOTAL_REVENUE || 0).toLocaleString()} revenue. ` +
        ...
        `Avg order: $${c.AVG_ORDER_VALUE || 0}. ${(c.UNIQUE_CUSTOMERS || 0).toLocaleString()} unique trial sites.`;
      fallbackData = [c];
  ...
  await logAction('chat_agent', 'chat_query', intent, null, {
    ...
    reason: `Chat query routed to ${teamDisplayLabel(team)} (intent: ${INTENT_DISPLAY_LABELS[intent] || intent})`,
  }, 0.90);
