...
async function executeAsUser(sql, binds = {}, username = null, options = {}) {
  ...
  try {
    connection = await getConnection();
    // A governed route must always provide an authenticated actor.  Never
    // silently escalate an absent or forged browser identity to an admin.
    if (!username) {
      const error = new Error('A governed actor is required for this query.');
      error.code = 'DEMO_ACTOR_REQUIRED';
      throw error;
    }
    ...
    if (connection) {
      try { await connection.execute('BEGIN sc_security_ctx.clear_user_context; END;'); } catch (e) { /* best effort cleanup */ }
      try { await connection.close(); } catch (e) { /* ignore */ }
    }
  }
}
