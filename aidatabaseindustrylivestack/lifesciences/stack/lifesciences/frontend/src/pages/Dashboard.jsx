            ...
            {inventory.length > 0 && (
              <div>
                <h4 className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2 flex items-center gap-1.5">
                  <MapPin size={12} /> Inventory by Cold Chain Site
                </h4>
              ...
              >
                ...
                <td className="py-2.5 px-3 text-center">
                  <span className={`momentum-badge momentum-${p.PEAK_MOMENTUM}`}>
                    {p.PEAK_MOMENTUM === 'mega_viral' ? 'CRITICAL' : p.PEAK_MOMENTUM?.replace('_', ' ')}
                  </span>
                </td>
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      <RegisterOraclePanel title="Quality & Supply Operations Dashboard">
...
ORDER BY avg_criticality DESC;`} />
          <div>
            ...
            <div className="grid grid-cols-3 gap-1.5">
              ...
              <DiagramBox label="Relational" sub="orders trial sites" color="#437C94" />
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">Seer Lifesciences Command Center</h2>
          <p className="text-sm text-[var(--color-text-dim)] mt-1">
            Real-time regulated supply, quality, and compliance intelligence powered by Oracle 26ai Converged Database.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        <StatCard iconClass="oj-fwk-icon-tree-document" label="Clinical Supply Orders" value={formatNumber(s.ORDERS_TOTAL)} subValue={`${formatNumber(s.ORDERS_30D)} last 30d`} color="#437C94" />
        <StatCard iconClass="oj-fwk-icon-view" label="Clinical Supply Exposure" value={formatCurrency(s.REVENUE_TOTAL)} subValue={`${formatCurrency(s.REVENUE_30D)} last 30d`} color="#4C825C" />
        <StatCard iconClass="oj-fwk-icon-message-warning" label="Critical Quality Signals" value={formatNumber(s.VIRAL_POSTS)} subValue={`${formatNumber(s.RISING_POSTS)} rising`} color="#C74634" />
        <StatCard iconClass="oj-fwk-icon-sortrelevancehigh" label="Products Under Watch" value={formatNumber(s.TRENDING_PRODUCTS)} subValue={`${formatNumber(s.POSTS_TOTAL)} total bulletins`} color="#AA643B" />
        <StatCard iconClass="oj-fwk-icon-users" label="AI Decisions Logged" value={formatNumber(s.AGENT_ACTIONS_TOTAL)} subValue={`${formatNumber(s.SHIPMENTS_IN_TRANSIT)} in transit`} color="#796087" />
      </div>
        ...
        <div className="glass-card p-5 lg:col-span-2">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={velocity || []}>
              ...
              />
              <Area type="monotone" dataKey="TOTAL_LIKES" stroke="#AA643B" fill="url(#gradLikes)" strokeWidth={2} name="Acknowledgements" />
              <Area type="monotone" dataKey="VIRAL_COUNT" stroke="#C74634" fill="url(#gradViral)" strokeWidth={2} name="Critical Quality Signals" />
            </AreaChart>
          </ResponsiveContainer>
          )}
        </div>

        {/* Clinical Supply Exposure by Product Category */}
      ...
      <div className="glass-card p-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-4">
          <h3 className="text-sm font-semibold flex items-center gap-2 flex-shrink-0">
            ...
            <span className="text-[10px] uppercase tracking-wider text-[var(--color-text-dim)] font-normal hidden sm:inline">
              - Regulatory and quality momentum (7 day)
            </span>
          </h3>
          ...
          <div className="relative flex-1 min-w-0">
            ...
            <input
              ...
              placeholder="Search regulated products or manufacturers..."
          ...
          {/* Manufacturer filter chips (populated from watched-product results) */}
        ...
        {loadingTrending ? (
          <p className="text-sm text-[var(--color-text-dim)]">Loading products under watch...</p>
        ) : (
          <TrendingTable
            products={trending}
      ...
      <div className="glass-card p-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex gap-2 flex-wrap">
            {[
              { label: 'Relational', desc: 'Orders and inventory', color: '#437C94' },
              { label: 'JSON', desc: 'Signal payloads', color: '#4C825C' },
              { label: 'Graph', desc: 'Quality propagation network', color: '#AA643B' },
              { label: 'Vector', desc: 'Semantic matching', color: '#796087' },
              { label: 'Spatial', desc: 'Cold-chain service coverage', color: '#4F7D7B' },
