...
const AGENT_COLORS = {
  ...
  commerce_agent: '#4C825C',
  master_orchestrator: '#796087',
  chat_agent: '#4F7D7B',
};

const TEAM_INFO = {
  SOCIAL_TREND_TEAM: { label: 'Regulatory & Quality Signal Agent', color: '#AA643B', iconClass: 'oj-fwk-icon-sortrelevancehigh', desc: 'Regulatory, quality, and supply signal analysis' },
  FULFILLMENT_TEAM:  { label: 'Cold-Chain Service Routing Agent', color: '#437C94', iconClass: 'oj-fwk-icon-tree-document', desc: 'Inventory, compliance, and routing' },
  COMMERCE_TEAM:     { label: 'Clinical Supply Exposure Agent', color: '#4C825C', iconClass: 'oj-fwk-icon-grid', desc: 'Orders, supply exposure, and trial-site demand' },
};

const DISPLAY_NAME_BY_ID = {
  SOCIAL_TREND_TEAM: 'Regulatory & Quality Signal Agent',
  TREND_AGENT: 'Regulatory & Quality Signal Agent',
  TREND_SQL_TOOL: 'Quality Signal SQL Tool',
  DETECT_TRENDS_TOOL: 'Quality Signal Detection Tool',
  INFLUENCER_NETWORK_TOOL: 'Signal Network Tool',
  FULFILLMENT_TEAM: 'Cold-Chain Service Routing Agent',
  FULFILLMENT_AGENT: 'Cold-Chain Service Routing Agent',
  CHECK_INVENTORY_TOOL: 'Inventory Check Tool',
  FULFILLMENT_ROUTE_TOOL: 'Cold-Chain Route Tool',
  COMMERCE_TEAM: 'Clinical Supply Exposure Agent',
  COMMERCE_AGENT: 'Clinical Supply Exposure Agent',
  COMMERCE_SQL_TOOL: 'Clinical Supply SQL Tool',
  LOG_DECISION_TOOL: 'Decision Audit Log Tool',
  chat_agent: 'AI Assistant',
  trend_detection_agent: 'Regulatory & Quality Signal Agent',
  inventory_agent: 'Cold-Chain Inventory Agent',
  fulfillment_agent: 'Cold-Chain Service Routing Agent',
  commerce_agent: 'Clinical Supply Exposure Agent',
  master_orchestrator: 'Governed Orchestrator',
};

const INTENT_LABELS = {
  commerce: 'clinical supply exposure',
  fulfillment: 'cold-chain service routing',
  trends: 'regulatory and quality signal analysis',
};

const INTENT_BADGE_LABELS = {
  commerce: 'clinical supply',
...
function displayName(value) {
  ...
  return DISPLAY_NAME_BY_ID[text] || text.replace(/_/g, ' ');
}

function displayToolName(value) {
  let text = String(value || '');
  for (const [id, label] of Object.entries(DISPLAY_NAME_BY_ID)) {
    text = text.replaceAll(id, label);
  }
  return text.replace(/intent:\s*commerce/gi, 'intent: clinical supply exposure');
}

function displayAuditSummary(payload) {
  if (!payload) return '';
  const raw = payload.reason || payload.product_name || payload.strategy || JSON.stringify(payload).slice(0, 120);
  return displayToolName(raw).replace(/intent:\s*(commerce|fulfillment|trends)/gi, (_, intent) => `intent: ${INTENT_LABELS[intent] || intent}`);
}

function displayEntityType(value) {
  const text = String(value || '');
  return INTENT_LABELS[text] || text.replace(/_/g, ' ');
}

const EXAMPLE_QUESTIONS = [
  { text: 'Which Seer products have the highest quality criticality?', iconClass: 'oj-fwk-icon-sortrelevancehigh', team: 'trends' },
  { text: 'Show clinical supply exposure by regulated product category', iconClass: 'oj-fwk-icon-grid', team: 'commerce' },
  { text: 'Find cold-chain sites with rising SLA pressure.', iconClass: 'oj-fwk-icon-tree-document', team: 'fulfillment' },
  { text: 'Show supply value exposure from signal-linked clinical orders.', iconClass: 'oj-fwk-icon-info', team: 'commerce' },
  { text: 'Find critical quality bulletins in the last 24 hours.', iconClass: 'oj-fwk-icon-sortrelevancehigh', team: 'trends' },
  { text: 'Which manufacturers are linked to critical quality signals?', iconClass: 'oj-fwk-icon-tree-document', team: 'fulfillment' },
  { text: 'Find nearest compliant cold-chain site with qPCR Respiratory Panel for a trial site in Miami.', iconClass: 'oj-fwk-icon-arrowtail-e', team: 'fulfillment' },
];
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-[var(--color-border)]" style={{ background: 'var(--color-surface)' }}>
      ...
      <div className="px-3 py-2 flex items-center gap-2" style={{ background: 'rgba(67,124,148,0.08)', borderBottom: '1px solid rgba(67,124,148,0.2)' }}>
        ...
        <span className="text-xs font-bold tone-ocean">Cold Chain Route - {product}</span>
      ...
      <div style={{ height: 240 }}>
        <MapContainer bounds={bounds} style={{ height: '100%', width: '100%', borderRadius: 0 }}
          ...
          {/* Route lines from buyer to each center */}
          {centers.map((c, i) => (
            <Polyline key={i}
          ...
          <CircleMarker center={[customer.lat, customer.lon]} radius={7}
            ...
            <MapTooltip permanent direction="bottom" offset={[0, 8]}
              ...
              <div style={{ fontSize: 10 }}>
                <strong>Trial Site</strong><br />{customer.city}, {customer.state}
              </div>
            </MapTooltip>
          </CircleMarker>
        </MapContainer>
...
function ChatAgent({ onActionLogged }) {
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      ...
      const result = await api.agents.chat(question, history);
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-[var(--color-text-dim)]">
              Ask questions - auto-routed to Regulatory & Quality, Cold-Chain Service Routing, or Clinical Supply Exposure agents
            </p>
          </div>
        </div>
        {messages.length > 0 && (
        ...
        {messages.length === 0 && (
          <div className="space-y-3 py-4">
            <div className="text-center mb-4">
              ...
              <p className="text-sm text-[var(--color-text-dim)]">Ask me anything about Seer Lifesciences regulatory signals, product risk, cold-chain inventory, or clinical supply orders.</p>
              <p className="text-[10px] text-[var(--color-text-dim)] mt-1">
                Powered by a local reasoning runtime with Oracle SQL and PL/SQL tools
              </p>
            </div>
              ...
              {EXAMPLE_QUESTIONS.map((eq, i) => (
                <div key={i} className="agent-console-example-tile">
                  <div className="agent-console-example-meta">
                    ...
                    <span className="text-[9px] text-[var(--color-text-dim)] uppercase">{INTENT_BADGE_LABELS[eq.team] || eq.team}</span>
                  </div>
...
const FALLBACK_PROFILE_INFO = {
  SC_LLAMA_PROFILE: {
    label: 'Local reasoning runtime',
    short: 'Local runtime',
    color: PROFILE_COLOR_MAP.SC_LLAMA_PROFILE,
    type: 'Local model + Oracle SQL',
  },
};
  ...
  useEffect(() => {
    api.agents.profiles().then(data => {
      ...
      (data?.profiles || [])
        ...
        .forEach((p, i) => {
          const label = /llama|ollama/i.test(p.model || '') ? 'Local reasoning runtime' : (p.model || getProfileDisplayLabel(p.name, i));
          info[p.name] = {
            label,
            short: /runtime/i.test(label) ? 'Local runtime' : label,
            color: PROFILE_COLOR_MAP[p.name] || PROFILE_COLORS[i % PROFILE_COLORS.length],
            type: /ollama|llama/i.test(`${p.provider || ''} ${p.type || ''} ${p.description || ''}`)
              ? 'Local model + Oracle SQL'
              : (p.type || p.description || 'Runtime profile'),
          };
        });
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-[var(--color-text)] leading-relaxed">
              These agent workflows run through the application layer, with the <span className="font-mono text-[var(--color-text)]">local reasoning runtime</span> handling reasoning
              and Oracle AI Database 26ai executing SQL and PL/SQL tools against live data. Three specialist paths route work across
              regulatory signal analysis, cold-chain service routing, and clinical supply exposure. Oracle provides the connected data access layer, runs the queries, and records decisions in
          ...
          <div className="flex flex-wrap gap-1.5">
            <FeatureBadge label="Local Reasoning Runtime" color="purple" />
            <FeatureBadge label="Governed SQL Profile" color="pink" />
          ...
          <SqlBlock code={`-- Agent runtime: app orchestration + local reasoning + Oracle AI Database 26ai
-- The app resolves intent -> routes to a specialist team -> executes SQL / PL/SQL in Oracle
-- The local runtime provides reasoning; Oracle provides governed data access, SQL execution, and audit logging

-- Example flow:
-- 1. Classify the request as Regulatory & Quality Signal Agent
  ...
  :product_id, :json_payload, 0.92, 'proposed');`} />
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { team: 'Regulatory & Quality Signal Path', agent: 'Signal Analysis Agent', tools: ['Quality Signal SQL Tool', 'Signal Detection Tool', 'Signal Network Tool', 'Decision Audit Log Tool'], color: '#A36472' },
                { team: 'Cold-Chain Service Routing Path', agent: 'Cold-Chain Routing Agent', tools: ['Clinical Supply SQL Tool', 'Inventory Check Tool', 'Cold-Chain Route Tool', 'Decision Audit Log Tool'], color: '#AA643B' },
                { team: 'Clinical Supply Exposure Path', agent: 'Clinical Supply Exposure Agent', tools: ['Clinical Supply SQL Tool', 'Decision Audit Log Tool'], color: '#4C825C' },
              ].map(t => (
                <div key={t.team} className="rounded-xl border border-[var(--color-border)] overflow-hidden">
                  <div className="px-2 py-1.5 text-center" style={{ background: `${t.color}12`, borderBottom: `2px solid ${t.color}44` }}>
                    <p className="text-[9px] font-semibold text-[var(--color-text)]">{t.team}</p>
                  </div>
                  <div className="p-2 space-y-1.5">
                    <span className="text-[9px] font-semibold text-[var(--color-text)]">{t.agent}</span>
                    <div className="space-y-0.5">
                      {t.tools.map(tool => (
                        <div key={tool} className="text-[8px] px-1.5 py-0.5 rounded bg-[var(--color-surface)] text-[var(--color-text-dim)]">
                          {tool}
                        </div>
                      ))}
                    </div>
          ...
          <div>
            ...
            <div className="space-y-1" style={{ fontSize: 9 }}>
              <DiagramBox label="Critical Signal Detected" sub="quality_signals · criticality_score >= 75" color="#AA643B" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="Regulatory & Quality Signal Path" sub="Signal reasoning + tool routing" color="#796087" />
              <div className="text-center text-[var(--color-text-dim)]">↓ calls PL/SQL tool</div>
              <DiagramBox label="detect_trending_products()" sub="Vector match · manufacturer graph centrality" color="#A36472" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="Cold-Chain Service Routing Path" sub="Capacity check · routing logic" color="#437C94" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="Clinical Supply Exposure Path" sub="Supply value · clinical orders · allocation" color="#4C825C" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
            ...
            <div className="rounded-lg p-2 text-[9px] mt-2" style={{ background: 'rgba(121,96,135,0.08)', border: '1px dashed rgba(121,96,135,0.3)', color: 'var(--color-text)' }}>
              ...
              The language runtime handles reasoning, while Oracle provides governed access to the connected data layer, SQL execution, PL/SQL tools, and durable action logging.
            </div>
          </div>
        </div>
      </RegisterOraclePanel>
      ...
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <JetGlyph iconClass="oj-fwk-icon-users" className="agent-console-page-glyph tone-plum" /> Seer Governed Agent Console
          </h2>
          <p className="text-sm text-[var(--color-text-dim)] mt-1">
            AI agent teams use a governed reasoning runtime for recommendations and Oracle for data, SQL execution, and action logging
          </p>
        </div>
        <div className="flex items-center gap-2">
          {/* AI Profile Selector */}
      ...
      <div className="glass-card p-5">
        ...
        <div className="space-y-2">
          ...
          {(!actions || actions.length === 0) && (
            <p className="text-sm text-[var(--color-text-dim)] text-center py-4">No agent actions yet. Ask an AI Agent question to create an auditable action.</p>
          )}
        </div>
      </div>
