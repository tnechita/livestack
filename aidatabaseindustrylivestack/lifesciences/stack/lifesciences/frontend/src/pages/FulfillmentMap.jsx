...
// ── Constants ─────────────────────────────────────────────────────────────────
const CARRIER_COLORS = { CryoLine: '#796087', TrialFreight: '#AA643B', SafeTemp: '#4F7D7B', 'Clinical Express': '#C74634' };
...
const LAYER_DEFS = [
  { key: 'customers',     label: 'Trial Sites',          color: '#4C825C' },
  { key: 'centers',       label: 'Cold-chain Service Centers',         color: '#437C94' },
  { key: 'routes',        label: 'Cold-chain Service Routes',      color: '#796087' },
  { key: 'zones',         label: 'Service Zones',        color: '#AA643B' },
  { key: 'h3',            label: 'Trial Demand Density',      color: '#C74634' },
...
function FulfillmentMapView({ centers, shipments, customers, zonesData, demandRegions, layers, setLayer }) {
  // H3 hexagonal density bins from trial-site lat/lng at resolution 4
  ...
  return (
    <div className="fulfillment-map-card">
      ...
      >
        ...
        {/* ── LAYER: Trial Demand Density (hexagonal site density heatmap) ── */}
            ...
            >
              <Tooltip sticky>
                <div style={{ fontSize: 11 }}>
                  <strong>{cell.count}</strong> trial sites<br />
        ...
        {/* ── LAYER: Cold-chain Service Routes (polylines colored by carrier) ── */}
        ...
        {/* ── LAYER: Trial Sites (small dots colored by tier) ── */}
      ...
      <div className="absolute bottom-4 left-4 z-[1000] text-[10px] bg-[var(--color-surface)]/90 px-3 py-2 rounded-lg border border-[var(--color-border)] pointer-events-none space-y-1.5"
        ...
        {layers.routes && (
          <div className="flex items-center gap-3">
            ...
            <span className="flex items-center gap-1"><span className="w-4 h-0.5 bg-brand-sienna inline-block" /> TrialFreight</span>
      ...
      <div className="absolute top-4 right-4 z-[1000] text-[10px] bg-[var(--color-surface)]/90 px-3 py-2 rounded-lg border border-[var(--color-border)] pointer-events-none space-y-0.5 text-right">
        ...
        {layers.h3 && (
          <div><span className="tone-sienna">H3 res-4</span> · {h3Cells.length} hexagons · {customers?.length ?? 0} trial sites</div>
        )}
        {layers.zones && (
...
export default function FulfillmentMap() {
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      <RegisterOraclePanel title="Cold-Chain Service Coverage">
        <div className="space-y-4">
          <div>
            <p className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">What's Happening</p>
            <p className="text-[var(--color-text)] leading-relaxed">
              Every cold-chain fulfillment site, service zone, trial-site address, and demand region is stored as an{' '}
              ...
              The H3 layer bins trial-site density client-side via{' '}
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            <FeatureBadge label="trial_site_tier" color="purple" />
          </div>
          <SqlBlock code={`-- Nearest compliant fulfillment site with stock
...
ORDER BY r.demand_index DESC;`} />
          <div>
            ...
            <p className="text-[var(--color-text)] leading-relaxed mb-2">
              ...
              transparently appends a WHERE clause - cold-chain logistics managers see only their regional sites,
  ...
  statement_types => 'SELECT,UPDATE'
          ...
          <div>
            ...
            <div className="space-y-1">
              <DiagramBox label="Cold-chain Service Centers" sub="SDO_GEOMETRY points · R-Tree index" color="#437C94" />
              ...
              <div className="text-center text-[var(--color-text-dim)] text-[9px]">↓</div>
              <DiagramBox label="Trial Sites" sub="new · standard · preferred · vip" color="#796087" />
              <div className="text-center text-[var(--color-text-dim)] text-[9px]">↓</div>
              <DiagramBox label="Trial Demand Density" sub="Uber H3 res-4 · demand heatmap" color="#AA643B" />
              <div className="text-center text-[var(--color-text-dim)] text-[9px]">↓</div>
      ...
      <div>
        <h2 className="text-2xl font-bold flex items-center gap-2">
          <JetGlyph iconClass="oj-fwk-icon-calendar-clock" className="fulfillment-page-glyph tone-teal" /> Cold-Chain Service Coverage
        </h2>
        <p className="text-sm text-[var(--color-text-dim)] mt-1">
          <span className="tone-teal">Oracle Spatial + VPD</span> for compliant fulfillment sites, demand regions, routes, and service zones using live data.
        </p>
      </div>
      ...
      {/* ── Cold-chain Service Centers Table ── */}
      <div className="glass-card p-5">
        <h3 className="text-sm font-semibold mb-3">Cold-chain Service Centers</h3>
      ...
      {/* ── Inventory Alerts - Compliance and Release Risk ── */}
      {(alerts || []).length > 0 && (
        <div className="glass-card p-5">
          <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
            <JetGlyph iconClass="oj-fwk-icon-message-warning" className="tone-red" /> Inventory Alerts - Compliance and Release Risk
          </h3>
