...
const ORDER_DETAIL_TABS = [
  ...
  { id: 'route', label: 'Cold-Chain Route' },
];
              ...
              {loadingDetail ? (
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {[
                      { label: 'Trial Site', value: `${order.FIRST_NAME} ${order.LAST_NAME}` },
                      { label: 'Location', value: `${order.CITY}, ${order.STATE_PROVINCE}` },
                      { label: 'Supply Exposure', value: formatCurrency(order.ORDER_TOTAL) },
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="text-[10px] text-[var(--color-text-dim)] uppercase tracking-wider border-b border-[var(--color-border)]">
                        ...
                        <th className="text-left py-1.5 px-2">Manufacturer</th>
                  ...
                  <>
                      ...
                      >
                        ...
                        <CircleMarker center={from} radius={8} fillColor="#437C94" fillOpacity={0.9} color="#fff" weight={2}>
                          <Tooltip permanent direction="top" offset={[0, -10]}
                            ...
                            <div className="route-map-tooltip__content">
                              ...
                              <span>{order.CENTER_NAME || 'Cold Chain Site'}</span>
                            </div>
                          </Tooltip>
                        </CircleMarker>
                        ...
                        <CircleMarker center={to} radius={8} fillColor="#4C825C" fillOpacity={0.9} color="#fff" weight={2}>
                          <Tooltip permanent direction="top" offset={[0, -10]}
                            className="route-map-tooltip">
                            <div className="route-map-tooltip__content">
                              <span className="route-map-tooltip__label">Trial Site</span>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      {[
                        ...
                        { label: 'Ship Cost', value: shipment?.SHIP_COST ? formatCurrency(shipment.SHIP_COST) : '-', color: '#437C94' },
...
-- SDO_GCDR.ELOC_DRIVE_TIME_POLYGON for isochrone zones` : `-- Distance between cold-chain site and trial site
...
export default function Orders() {
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      <RegisterOraclePanel title="Clinical Supply Orders & Deviations">
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-[var(--color-text)] leading-relaxed">
              Clinical supply orders are stored in a classic <span className="tone-ocean font-mono">relational schema</span> - ACID transactions, foreign keys, and referential integrity.
...
FROM orders o WITH UPDATE;`} />
          ...
          <div>
            ...
            <p className="text-xs text-[var(--color-text-dim)] mb-2 leading-relaxed">
              Products with nested inventory across all cold-chain fulfillment sites. One document, two tables.
            </p>
...
-- Returns: {"_id":1, "status":"shipped", "items":[...]}`} />
          ...
          <div className="rounded-lg p-3 space-y-2" style={{ background: 'var(--color-surface-muted)', border: '1px solid var(--color-border)' }}>
            ...
            <DiagramBox label="products + inventory" sub="Product catalog + fulfillment-site stock levels" color="#437C94" />
            <div className="text-center text-[10px] text-[var(--color-text)]">↕ CREATE JSON RELATIONAL DUALITY VIEW</div>
  ...
  statement_types => 'SELECT');`} />
      ...
      <div>
        <h2 className="text-2xl font-bold flex items-center gap-2">
          <ShoppingCart className="tone-ocean" /> Seer Clinical Supply Orders & Deviations
        </h2>
        <p className="text-sm text-[var(--color-text-dim)] mt-1">
          ...
          Deviations are represented through signal-linked orders, quality signals, and agent actions.
        </p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-[11px] text-[var(--color-text-dim)] uppercase tracking-wider border-b border-[var(--color-border)] bg-[var(--color-surface)]/50">
                <th className="text-left py-3 px-4">Supply Order #</th>
                <th className="text-left py-3 px-4">Trial Site</th>
                <th className="text-left py-3 px-4">Location</th>
                <th className="text-center py-3 px-4">Status</th>
                <th className="text-right py-3 px-4">Items</th>
                <th className="text-right py-3 px-4">Supply Exposure</th>
                <th className="text-center py-3 px-4">Signal Linked</th>
                <th className="text-left py-3 px-4">Fulfillment Site</th>
                <th className="text-left py-3 px-4">Created</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={9} className="py-8 text-center text-[var(--color-text-dim)]">Loading clinical supply orders...</td></tr>
              ) : (orders || []).length === 0 ? (
                <tr><td colSpan={9} className="py-8 text-center text-[var(--color-text-dim)]">No clinical supply orders found</td></tr>
              ) : (
                (orders || []).map(o => {
                  ...
                  return [
                    <tr key={o.ORDER_ID}
                      ...
                      <td className="py-3 px-4 text-center">
                        {o.SOCIAL_DRIVEN ? (
                          <span className="tone-rose text-xs font-semibold">Signal Linked</span>
                        ) : (
