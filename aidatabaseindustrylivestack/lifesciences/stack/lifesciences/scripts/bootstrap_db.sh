...
echo ">>> Life Sciences bootstrap starting inside db container..."
...
echo ">>> Loading and probing the fixed 384/FLOAT32/DENSE embedding model before vector SQL compilation..."
sqlplus -L -s "$APP_CONNECT" @${APP_DIR}/db/schema/04a_vector_model_load_and_probe.sql
...
@/tmp/08_agents_functions.sql
@${APP_DIR}/db/schema/11_oml_models.sql
@${APP_DIR}/db/schema/14_lifesciences_vector_contract.sql
@${APP_DIR}/db/schema/13_dataset_generation_lifecycle.sql
COMMIT;
EXIT
SQL
