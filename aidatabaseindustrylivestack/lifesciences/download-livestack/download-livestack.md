...
1. Download the package named `livestack-lifesciences.zip` from the provided LiveStack distribution location.
2. Save the file to your machine.

Expected result:
- You have `livestack-lifesciences.zip` available on your machine.

## Task 2: Move the package and prepare environment settings

> **Note:** Do not extract or run the stack from your `Downloads` folder. Create a new working directory and move `livestack-lifesciences.zip` there first.
...
4. Move the downloaded package from `Downloads` into this directory:
    ```bash
<copy>
mv ~/Downloads/livestack-lifesciences.zip .
<copy>
```
...
5. Extract the package:
    ```bash
<copy>
unzip livestack-lifesciences.zip
<copy>
```
...
Expected result:
- You are inside the `lifesciences` directory.
