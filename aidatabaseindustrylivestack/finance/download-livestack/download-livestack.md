...
This lab shows how to run the Finance LiveStack in your own environment using the portable stack package and Podman Compose.
...
1. Download the package named `livestack-finance.zip` from the provided LiveStack distribution location.
2. Save the file to your machine.

Expected result:
- You have `livestack-finance.zip` available on your machine.

## Task 2: Move the package and prepare environment settings

> **Note:** Do not extract or run the stack from your `Downloads` folder. Create a new working directory and move `livestack-finance.zip` there first.
...
Expected result:
- You are inside the extracted `finance` directory.
- The folder contains `compose.yml`, `.env`, the `frontend`, `backend`, `db`, and `scripts` folders.
...
A portable runbook is what turns a LiveStack demo into a repeatable field asset. Podman Compose startup, health checks, and clear folder layout instructions reduce setup drift and let users replay the same finance story in their own environment.
