...
const CUSTOMER_STORAGE_KEY = 'stateLocalGovernmentLivestack.customerName';
const CUSTOMER_ALIASES = [
  'Seer State and Local Government',
  'State and Local Government Service Operations LiveStack',
  'State and Local Government Service Operations',
  'State and Local Government Demo Data',
  'Seer Public Sector',
  'Seer Government',
  'Seer Agency Operations',
];
...
const NAV_ITEMS = [
  ...
  { id: 'dashboard', label: 'Public Service Command Center', iconClass: 'oj-fwk-icon oj-fwk-icon-grid', featureTags: ['In-Memory', 'Native JSON'] },
  { id: 'social', label: 'Resident Demand Signals', iconClass: 'oj-fwk-icon oj-fwk-icon-sortrelevancehigh', featureTags: ['Vector Search', 'VPD'] },
  { id: 'graph', label: 'Community Partner Network', iconClass: 'oj-fwk-icon oj-fwk-icon-node-expand', featureTags: ['Property Graph', 'SQL/PGQ'] },
  { id: 'fulfillment', label: 'Service Access & Coverage Map', iconClass: 'oj-fwk-icon oj-fwk-icon-calendar-clock', featureTags: ['Oracle Spatial', 'VPD'] },
  { id: 'orders', label: 'Service Request Workbench', iconClass: 'oj-fwk-icon oj-fwk-icon-tree-document', featureTags: ['JSON Duality', 'VPD'] },
  { id: 'oml', label: 'Demand & Capacity Analytics', iconClass: 'oj-fwk-icon oj-fwk-icon-view', featureTags: ['OML', 'Vector K-Means'] },
  { id: 'askdata', label: 'Ask State and Local Government Data', iconClass: 'oj-fwk-icon oj-fwk-icon-magnifier', featureTags: ['Natural Language SQL', 'Live Schema'] },
  { id: 'agents', label: 'Public Service AI Agent Console', iconClass: 'oj-fwk-icon oj-fwk-icon-users', featureTags: ['PL/SQL Tools', 'JSON Audit'] },
];
...
function replaceCustomerTerms(value, profile) {
  ...
  let next = value.replace(/\bseer-state-local-government\.example\b/gi, profile.customerDomain);
  for (const alias of CUSTOMER_ALIASES) {
    next = next.replace(new RegExp(`\\b${alias.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\b`, 'g'), profile.customerName);
  }
...
function hasReplaceableCustomerTerms(value) {
  return typeof value === 'string' && (
    /\bseer-state-local-government\.example\b/i.test(value) ||
    /\b(?:Seer State and Local Government|State and Local Government Service Operations LiveStack|State and Local Government Service Operations|State and Local Government Demo Data|Seer Public Sector|Seer Government|Seer Agency Operations|Seer)\b/.test(value)
  );
}
...
function OracleBrand({ profile }) {
  const title = replaceCustomerTerms(`${CUSTOMER_NAME} LiveStack`, profile);
  return (
    <button
      type="button"
    ...
    <div className="customer-name-tool">
      ...
      {isOpen && (
        <form className="customer-name-tool__panel" onSubmit={submit}>
          ...
          <p className="customer-name-tool__hint">Replaces Seer and State and Local Government labels on visible screens.</p>
        </form>
      )}
    </div>
  );
...
function IdentityBoundPage({ activePage, onNavigate, onIdentityChange }) {
  const { currentUser } = useUser();
  const identityKey = currentUser?.USERNAME || 'viewer_sam';
  ...
  if (activePage === 'welcome') {
    return <Welcome key={`welcome:${currentUser?.USERNAME || 'viewer_sam'}`} onNavigate={onNavigate} />;
  }

  const PageComponent = PAGES[activePage];
  if (!PageComponent) return null;
  const pageProps = activePage === 'datamodel' ? { onNavigate } : {};

  // Remount the active scene whenever the Oracle identity changes. This
  // refetches every scene and clears prior-user detail, search, and chat state.
  return <PageComponent key={`${activePage}:${currentUser?.USERNAME || 'viewer_sam'}`} {...pageProps} />;
}
          ...
          <div className="app-shell">
            ...
            <div className="app-main">
              <header className="app-topbar">
                ...
                <JetButton
                  label="Use Your Own Public Service Data"
              ...
              <main className="app-content">
                <div className="app-page-frame">
                  <IdentityBoundPage
                    ...
                    onIdentityChange={refreshActiveDataset}
                  />
                </div>
              </main>
            </div>
