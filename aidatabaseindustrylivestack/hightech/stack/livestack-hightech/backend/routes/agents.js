...
const STATIC_TEAMS = [
  {
    ...
    STATUS: 'ENABLED',
    DESCRIPTION: 'Ollama-backed customer and developer signal analysis over live technology product intelligence data.',
  },
  {
    TEAM_NAME: 'PRODUCT_AVAILABILITY_AGENT',
    STATUS: 'ENABLED',
    DESCRIPTION: 'Ollama-backed product availability analysis using capacity and routing context.',
  },
  {
    TEAM_NAME: 'PRODUCT_OPERATIONS_AGENT',
    STATUS: 'ENABLED',
    DESCRIPTION: 'Ollama-backed product operations analysis using customer commitments and commitment-value context.',
  },
];

const TOOL_DISPLAY_LABELS = {
  'Ollama llama3.2': 'Ollama llama3.2',
  'detect_trending_products()': 'Demand Volatility Detector',
  'get_influencer_network()': 'Product Signal Graph Traversal',
  customer_lookup: 'Customer Commitment Lookup',
  'find_best_fulfillment()': 'Order Promising Route Check',
  'check_product_inventory()': 'BOM and Component Capacity Check',
  COMMITMENT_SQL_WORKBENCH: 'Commitment SQL Workbench',
  COMMITMENT_CATEGORY_ANALYSIS: 'Commitment Category Analysis',
  AGENT_FALLBACK_GUARDRAIL: 'Fallback Guardrail',
};
...
function normalizeToolDisplayName(toolName) {
  ...
  return TOOL_DISPLAY_LABELS[raw] || TOOL_DISPLAY_LABELS[base] || titleCaseToolName(base);
}
...
function normalizeToolEntry(entry = {}) {
  return {
    ...entry,
    tool_key: entry.tool,
    tool: normalizeToolDisplayName(entry.tool),
  };
}
...
async function buildAgentContext(teamName) {
  if (teamName === 'TECH_SIGNAL_AGENT') {
    ...
    return {
      instructions: 'Focus on high-tech products, developer advocates, urgent signals, and concrete metrics.',
      context: {
        team: teamName,
  ...
  if (teamName === 'PRODUCT_AVAILABILITY_AGENT') {
    ...
    return {
      instructions: 'Focus on capacity risk, routing, and practical product availability actions.',
      context: {
        team: teamName,
  ...
  return {
    instructions: 'Focus on customer commitments, commitment value, product-signal attribution, and operational trends.',
    context: {
      team: teamName,
      productOperationsSummary: summary.rows?.[0] || {},
...
function fallbackAgentSummary(teamName, context) {
  if (teamName === 'TECH_SIGNAL_AGENT') {
    ...
    if (!products.length) {
      return context.trend_summary || 'No high-demand high-tech products found in the current window.';
    }
    return products
      .slice(0, 3)
      .map((product) => {
  ...
  if (teamName === 'PRODUCT_AVAILABILITY_AGENT') {
    ...
    if (!alerts.length) {
      return 'No current low-capacity high-tech product alerts were found.';
    }
    return alerts
      .slice(0, 3)
      .map((item) =>
  ...
  const summary = context.productOperationsSummary || {};
  const totalOrders = summary.TOTAL_ORDERS || 0;
  const totalRevenue = summary.TOTAL_REVENUE || 0;
  const socialOrders = summary.SOCIAL_ORDERS || 0;
  const socialRevenue = summary.SOCIAL_REVENUE || 0;
  return `Last 30 days: ${totalOrders.toLocaleString()} customer commitments, $${totalRevenue.toLocaleString()} commitment value, ${socialOrders.toLocaleString()} product-signal-driven commitments, $${socialRevenue.toLocaleString()} product-signal commitment value.`;
}
...
function getLastConversationTeam(history = []) {
  if (!Array.isArray(history)) return null;
  const priorAgentMessage = [...history].reverse().find((message) => message?.role === 'assistant' && message?.team);
  return STATIC_TEAMS.some((entry) => entry.TEAM_NAME === priorAgentMessage?.team)
    ? priorAgentMessage.team
    : null;
}
...
async function askAgent(teamName, question, conversationContext = '') {
  ...
  const questionWithContext = conversationContext
    ? `${question}\n\nPrior conversation (use only to resolve follow-up references):\n${conversationContext}`
    : question;
  try {
    return await Promise.race([
      summarizeContext({ question: questionWithContext, instructions, context }),
...
// Runs the TECH_SIGNAL_AGENT to identify urgent high-tech product demand.
...
router.post('/detect-trends', async (req, res) => {
  ...
  try {
    ...
    const trendText = trendResult.rows[0]?.RESULT || 'No high-demand high-tech products found';

    // 2. Get top trending high-tech products for per-service action logging
    ...
    try {
      agentAnalysis = await Promise.race([
        askAgent('TECH_SIGNAL_AGENT',
          `Identify the top trending high-tech products and developer advocates from the last ${hours} hours ` +
          `using the detect trending high-tech products tool with minimum urgency score ${threshold}`),
    ...
    for (const p of products) {
      ...
      await logAction('trend_detection_agent', 'detect_trends', 'product', p.PRODUCT_ID, {
        ...
        window_hours:  hours,
        reason: `${p.PEAK_MOMENTUM} high-tech product with ${p.MENTION_COUNT} customer and developer signal mentions and urgency ${p.AVG_VIRALITY}`,
      }, confidence);
      loggedActions.push({ product: p.PRODUCT_NAME, urgency: p.AVG_VIRALITY });
    }
    ...
    await logAction('trend_detection_agent', 'trend_analysis_complete', 'social_posts', null, {
      ...
      product_lines_found:  products.length,
    ...
    res.json({
      message:      `Trend detection complete - ${products.length} urgent high-tech products identified in last ${hours}h`,
      trending:     trendText,
...
// Full orchestration: trend detection → inventory check → product operations attribution.
  ...
  try {
    ...
    try {
      trendAnalysis = await Promise.race([
        askAgent('TECH_SIGNAL_AGENT',
          'What high-tech products are trending right now based on customer and developer signal activity in the last 48 hours'),
        new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 10000)),
      ]);
    } catch (e) {
    ...
    await logEvent('trend_detected', 'master_orchestrator', {
      phase: 'trend_detection', product_lines_found: topProducts.length,
    });
    ...
    try {
      fulfillmentAnalysis = await Promise.race([
        askAgent('PRODUCT_AVAILABILITY_AGENT',
          'Which trending high-tech products have critically low capacity and need immediate intervention'),
        new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 10000)),
      ]);
    } catch (e) {
      logOptionalAgentWarning('Product availability agent skipped', e);
    }
    ...
    for (const inv of criticalInventory) {
      await logAction('inventory_agent', 'inventory_alert', 'inventory', inv.PRODUCT_ID, {
        ...
        strategy:           `Pre-position stock at ${inv.CENTER_NAME} - trending high-tech product with ${inv.STOCK_STATUS} inventory`,
    ...
    // ── PHASE 3: Product Operations Attribution ────────────────────────────────────────
    const productOperationsResult = await db.execute(
      `SELECT /*+ NO_PARALLEL */
    ...
    const productOperations = productOperationsResult.rows[0] || {};

    // Best-effort LLM product operations analysis
    let productOperationsAnalysis = null;
    try {
      productOperationsAnalysis = await Promise.race([
        askAgent('PRODUCT_OPERATIONS_AGENT',
          'Summarize product-signal-driven customer commitments and commitment value from the last 7 days'),
        new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 10000)),
      ]);
    } catch (e) {
      logOptionalAgentWarning('Product operations agent skipped', e);
    }

    const socialPct = productOperations.TOTAL_ORDERS > 0
      ? ((productOperations.SOCIAL_ORDERS / productOperations.TOTAL_ORDERS) * 100).toFixed(1)
      : 0;

    await logAction('master_orchestrator', 'product_operations_attribution', 'orders', null, {
      total_orders:    productOperations.TOTAL_ORDERS,
      social_orders:   productOperations.SOCIAL_ORDERS,
      total_revenue:   productOperations.TOTAL_REVENUE,
      social_revenue:  productOperations.SOCIAL_REVENUE,
      social_pct:      `${socialPct}%`,
      avg_order_value: productOperations.AVG_ORDER_VALUE,
      reason: `${socialPct}% of customer commitments ($${(productOperations.SOCIAL_REVENUE || 0).toLocaleString()}) attributed to product signals in last 7 days`,
    }, 0.93);
    allActions.push({ phase: 'product operations', social_pct: socialPct });

    await logEvent('product_operations_analysis_complete', 'master_orchestrator', {
      phase: 'product_operations_attribution',
      social_orders: productOperations.SOCIAL_ORDERS,
      signal_service_value: productOperations.SOCIAL_REVENUE,
    });
    ...
    res.json({
      message: `Full cycle complete - ${topProducts.length} trends · ${criticalInventory.length} capacity alerts · ${socialPct}% product-signal-driven customer commitments`,
      phases: {
        trends: {
          product_lines_found: topProducts.length,
        ...
        productOperations: {
          total_solution_orders: productOperations.TOTAL_ORDERS,
          signal_solution_orders: productOperations.SOCIAL_ORDERS,
          signal_service_value: productOperations.SOCIAL_REVENUE,
          social_pct:     `${socialPct}%`,
          analysis:       productOperationsAnalysis,
        },
      },
      actions:      allActions,
      distribution: distResult.rows,
...
router.post('/ask', async (req, res) => {
  try {
    ...
    const validTeams = ['TECH_SIGNAL_AGENT', 'PRODUCT_AVAILABILITY_AGENT', 'PRODUCT_OPERATIONS_AGENT'];
...
router.post('/trends', async (req, res) => {
  try {
    ...
    const q = question || 'What high-tech products are trending right now based on customer and developer signal activity';
    const response = await askAgent('TECH_SIGNAL_AGENT', q);
    res.json({ team: 'TECH_SIGNAL_AGENT', question: q, response });
  } catch (err) {
...
// ── POST /api/agents/fulfillment - ask the product availability agent ──
router.post('/fulfillment', async (req, res) => {
  try {
    const { question } = req.body;
    const q = question || 'Which trending high-tech products have critically low capacity';
    const response = await askAgent('PRODUCT_AVAILABILITY_AGENT', q);
    res.json({ team: 'PRODUCT_AVAILABILITY_AGENT', question: q, response });
  } catch (err) {
    console.error('Product availability agent error:', err);
    res.status(500).json({ error: err.message });
  }
});

// ── POST /api/agents/product operations - ask the product operations agent ──
router.post('/product-operations', async (req, res) => {
  try {
    const { question } = req.body;
    const q = question || 'How many customer commitments were placed in the last 24 hours and what is the total commitment value';
    const response = await askAgent('PRODUCT_OPERATIONS_AGENT', q);
    res.json({ team: 'PRODUCT_OPERATIONS_AGENT', question: q, response });
  } catch (err) {
    console.error('Product operations agent error:', err);
    res.status(500).json({ error: err.message });
  }
});
...
router.post('/chat', async (req, res) => {
  ...
  const priorTeam = getLastConversationTeam(history);
  const startTime = Date.now();

  // ── Step 1: Auto-detect intent and pick agent team ──
  let team = 'PRODUCT_OPERATIONS_AGENT';
  let intent = 'product operations';
  let toolsUsed = [];
  ...
  const inventoryStrong = ['capacity', 'product availability', 'inventory', 'reorder', 'replenish', 'out of capacity'];
  const productOperationsStrong = ['product value', 'commitment value', 'customer commitment', 'customer commitments', 'solution order', 'request total', 'order total'];

  // Weak signals - worth 1 point each (ambiguous, could relate to multiple intents)
  const trendWeak = ['trend', 'signal', 'product signal', 'post', 'engagement', 'views', 'likes', 'shares', 'sentiment'];
  const inventoryWeak = ['stock', 'route', 'routing', 'center', 'supply', 'logistics', 'completion', 'nearest', 'distance'];
  const productOperationsWeak = ['commitment', 'commitments', 'request', 'enterprise buyer', 'value', 'category', 'program', 'service', 'total'];
  ...
  const productOperationsScore = productOperationsStrong.filter(k => qLower.includes(k)).length * 3
                      + productOperationsWeak.filter(k => qLower.includes(k)).length;

  if (trendScore >= inventoryScore && trendScore >= productOperationsScore && trendScore > 0) {
    team = 'TECH_SIGNAL_AGENT'; intent = 'trends';
  } else if (inventoryScore > trendScore && inventoryScore >= productOperationsScore) {
    team = 'PRODUCT_AVAILABILITY_AGENT'; intent = 'fulfillment';
  } else if (trendScore === 0 && inventoryScore === 0 && productOperationsScore === 0 && priorTeam) {
    team = priorTeam;
    intent = priorTeam === 'TECH_SIGNAL_AGENT'
      ? 'trends'
      : priorTeam === 'PRODUCT_AVAILABILITY_AGENT'
        ? 'fulfillment'
        : 'product operations';
  }
  ...
  try {
    agentResponse = await Promise.race([
      askAgent(team, q, conversationContext),
  ...
  try {
    if (intent === 'trends') {
      ...
      fallbackResult = trendRes.rows[0]?.RESULT || 'No high-demand high-tech products found';
      ...
      // Check for developer advocate-specific questions
      ...
      const productPatterns = [
        /["']([^"']+)["']/,                                                           // quoted: "Virtual Edge Computing Visit"
        /(?:inventory|stock|check)\s+(?:for|of|on)\s+(?:the\s+)?(.+?)(?:\s+across|\s+at|\s+in|\s*\??\s*$)/i,
        /(?:ship|deliver|send|route)\s+(?:the\s+)?(.+?)(?:\s+to\s+|\s+for\s+)/i,     // "ship same-day innovation hub slot to..."
      ...
      if (productName && (cityMatch || emailMatch)) {
        // Spatial routing - find best product availability center
        let customerEmail = emailMatch ? emailMatch[0] : null;

        // If user gave a city name, look up a synthetic enterprise buyer email in that city
        ...
        if (customerEmail) {
          try {
            ...
            fallbackResult = routeRes.rows[0]?.RESULT || 'No order promising route found';
            toolsUsed.push({ tool: 'find_best_fulfillment()', params: { synthetic_enterprise_buyer: customerEmail, product: productName }, status: 'success' });
            ...
            fallbackResult = invRes.rows[0]?.RESULT || 'No component capacity data found';
        ...
        fallbackResult = `Product availability overview: ${invRes.rows.length} active supply sites across fab operations, contract manufacturing, and customer commitment capacity.`;
        toolsUsed.push({ tool: 'COMMITMENT_SQL_WORKBENCH', status: 'success' });
      }

    } else {
      // Product Operations - general orders/revenue queries
      const productOperationsRes = await db.execute(
        `SELECT COUNT(*) AS total_orders,
                COUNT(CASE WHEN social_source_id IS NOT NULL THEN 1 END) AS social_orders,
                ROUND(SUM(order_total), 2) AS total_revenue,
                ROUND(SUM(CASE WHEN social_source_id IS NOT NULL THEN order_total ELSE 0 END), 2) AS social_revenue,
      ...
      const c = productOperationsRes.rows[0] || {};
      const socialPct = c.TOTAL_ORDERS > 0 ? ((c.SOCIAL_ORDERS / c.TOTAL_ORDERS) * 100).toFixed(1) : '0';

      fallbackResult = `Last 30 days: ${(c.TOTAL_ORDERS || 0).toLocaleString()} customer commitments, $${(c.TOTAL_REVENUE || 0).toLocaleString()} commitment value. ` +
        `${socialPct}% product-signal-driven ($${(c.SOCIAL_REVENUE || 0).toLocaleString()}). ` +
        `Avg commitment: $${c.AVG_ORDER_VALUE || 0}. ${(c.UNIQUE_CUSTOMERS || 0).toLocaleString()} enterprise buyers.`;
      fallbackData = [c];
      toolsUsed.push({ tool: 'COMMITMENT_SQL_WORKBENCH', status: 'success' });
      ...
      if (qLower.includes('category') || qLower.includes('breakdown')) {
        ...
        toolsUsed.push({ tool: 'COMMITMENT_CATEGORY_ANALYSIS', status: 'success' });
      }
    }
  } catch (toolErr) {
    toolsUsed.push({ tool: 'AGENT_FALLBACK_GUARDRAIL', status: 'error', reason: toolErr.message });
  }
  ...
  const normalizedToolsUsed = toolsUsed.map(normalizeToolEntry);

  const toolHistory = normalizedToolsUsed.slice(0, 5).map((entry) => ({
