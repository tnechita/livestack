...
const { getJob } = require('../lib/importJobs');
...
// Dataset execution is authorized from the Oracle-derived request identity.
// The explicit command header and same-origin browser metadata provide a
// separate intent boundary before protected bodies are parsed.
...
router.get('/template', async (req, res) => {
  ...
  try {
    ...
    const fileName = archive.fileName || `high-tech-product-intelligence-import-template-${version}.zip`;
...
router.get('/status/:jobId', requireDemoIdentity, async (req, res) => {
  ...
  try {
    ...
    const status = await getJob(jobId.trim());
