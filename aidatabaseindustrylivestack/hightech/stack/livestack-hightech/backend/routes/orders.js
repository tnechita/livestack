...
const CANCELLATION_REASONS = [
  'Customer requested configuration change',
  'Component shortage',
  'Duplicate order',
  'Delivery date missed',
  'Pricing approval expired',
  'Customer project delayed',
];

function cancellationReason(order) {
  if (String(order?.ORDER_STATUS || '').toLowerCase() !== 'cancelled') return null;
  const index = Math.max(0, (Number(order.ORDER_ID) || 1) - 1) % CANCELLATION_REASONS.length;
  ...
  try {
    ...
    const result = await db.executeAsUser(`
      SELECT o.order_id, o.order_status, o.order_total, o.shipping_cost,
             o.demand_score, o.estimated_delivery, o.actual_delivery, o.created_at,
    ...
    res.json(result.rows.map((order) => ({
      ...order,
      CANCELLATION_REASON: cancellationReason(order),
    })));
  } catch (err) {
...
router.get('/:id', async (req, res) => {
  try {
    ...
    ord.CANCELLATION_REASON = cancellationReason(ord);
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({
        ...
        error: 'Order not found in native duality view',
      });
    }
    ...
    res.json({
      ...
      readOnly: true,
      sql: ORDER_DUALITY_SQL.trim(),
      executedSql: ORDER_DUALITY_SQL.trim(),
      binds: { id: orderId },
      document: doc
    });
  } catch (err) {
    console.error('Order duality view error:', err);
    res.status(503).json({
      source: 'ORDERS_DV',
      sourceMode: 'unavailable',
      code: err.code || 'NATIVE_DUALITY_VIEW_UNAVAILABLE',
      error: 'Native Oracle JSON Relational Duality View ORDERS_DV is unavailable.',
    });
  }
});
