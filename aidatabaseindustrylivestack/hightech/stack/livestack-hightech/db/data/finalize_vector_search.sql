...
BEGIN
    SAVEPOINT hightech_vector_rebuild;
    ...
    IF evidence_is_ready THEN
        DBMS_OUTPUT.PUT_LINE(
            ...
            v_post_vectors || ' product signals, ' ||
    ...
    DBMS_OUTPUT.PUT_LINE(
        ...
        v_product_vectors || ' products, ' ||
        v_total_rows || ' product signals, ' ||
        v_semantic_matches || ' deterministic matches.'
    );
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK TO hightech_vector_rebuild;
        RAISE;
END;
/

PROMPT High Tech Oracle AI Vector Search artifacts are ready.
