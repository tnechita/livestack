...
DECLARE
  active_center_count PLS_INTEGER;
  zone_count          PLS_INTEGER;
  boundary_count      PLS_INTEGER;
BEGIN
  SELECT COUNT(*)
   ...
   WHERE is_active = 1
     AND location IS NOT NULL;
  SELECT COUNT(*) INTO zone_count FROM fulfillment_zones;
  SELECT COUNT(*)
    INTO boundary_count
    FROM fulfillment_zones
   WHERE zone_boundary IS NOT NULL;

  IF active_center_count < 1
     OR boundary_count <> zone_count
     OR zone_count <> active_center_count * 4 THEN
    RAISE_APPLICATION_ERROR(
      -20421,
      'Fresh-bootstrap fulfillment-zone geometry is incomplete'
    );
  END IF;
END;
/
