...
/*
-- Find developer advocates within 2 hops of an urgent signal post author
...
FROM GRAPH_TABLE ( influencer_network
    ...
    WHERE v1.handle = '@edge_stack_ava'
    COLUMNS (
        v2.handle,
        v2.influence_score,
        v2.follower_count
...
-- Find shortest propagation path between two developer advocates
SELECT *
FROM GRAPH_TABLE ( influencer_network
    MATCH SHORTEST (a IS influencer) -[e IS connects_to]->{1,5} (b IS influencer)
    WHERE a.handle = '@telemetry_mark'
...
-- Technology portfolio propagation: which advocates propagate a technology portfolio through the network
...
FROM GRAPH_TABLE ( influencer_network
    ...
    WHERE b.brand_name = 'QuantumCore Systems'
    COLUMNS (
        v1.handle, v2.handle,
        e1.relationship_type,
        e2.connection_type,
