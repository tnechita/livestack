 ...
 * The VPD package and functions run as the schema owner. Unified Audit is
 * installed by ADMIN through 16_hightech_unified_audit_admin.sql.
 */
...
-- Canonical High Tech row-level security moved to the private-context phases:
--   06b_hightech_vpd.sql (trusted package + policies)
--   06a_hightech_app_context_admin.sql (ADMIN-owned context binding)
-- Keep this marker as the bootstrap boundary for the administrator role DDL,
-- but do not execute the retired package-global implementation below.
PROMPT Legacy package-global VPD section retired; use 06a/06b canonical scripts.
EXIT SUCCESS
