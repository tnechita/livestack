...
BEGIN
    ...
    SELECT p_post_id,
           pe.product_id,
           1 - VECTOR_DISTANCE(v_post_vector, pe.embedding, COSINE) AS sim_score,
           ROW_NUMBER() OVER (
               ORDER BY VECTOR_DISTANCE(v_post_vector, pe.embedding, COSINE), pe.product_id
           ),
           'vector'
...
BEGIN
    OPEN v_results FOR
        SELECT p.product_id,
               ...
               p.unit_price,
               b.brand_name,
               VECTOR_DISTANCE(pe.embedding, p_query_vec, COSINE) AS vector_distance,
               1 - VECTOR_DISTANCE(pe.embedding, p_query_vec, COSINE) AS similarity
        FROM product_embeddings pe
        JOIN products p ON pe.product_id = p.product_id
        JOIN brands   b ON p.brand_id    = b.brand_id
        ORDER BY VECTOR_DISTANCE(pe.embedding, p_query_vec, COSINE), pe.product_id
