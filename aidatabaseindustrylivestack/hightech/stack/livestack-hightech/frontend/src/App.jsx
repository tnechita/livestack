...
const CUSTOMER_STORAGE_KEY = 'hightechLivestack.customerName';
const LEGACY_HIGH_TECH_PRODUCT_INTELLIGENCE = ['High', 'Tech', 'Product', 'Intelligence'].join(' ');
const CUSTOMER_ALIASES = [
  LEGACY_HIGH_TECH_PRODUCT_INTELLIGENCE,
  'High Tech Demo Data',
  'Seer Tech demo data',
  'Seer Tech product intelligence',
  'high-tech product intelligence',
  'Seer Equity Bank',
  'Seer Equity',
  'Seer Bank',
  'Seer',
...
const NAV_ITEMS = [
  ...
  { id: 'fulfillment', label: 'Supply & Commitment Map', iconClass: 'oj-fwk-icon oj-fwk-icon-calendar-clock', featureTags: ['Oracle Spatial', 'Virtual Private Database'] },
  { id: 'orders', label: 'Customer Commitments', iconClass: 'oj-fwk-icon oj-fwk-icon-tree-document', featureTags: ['JSON Duality', 'Virtual Private Database'] },
  { id: 'oml', label: 'Oracle Machine Learning Product Intelligence', iconClass: 'oj-fwk-icon oj-fwk-icon-view', featureTags: ['Oracle Machine Learning', 'Vector K-Means'] },
  { id: 'askdata', label: 'Ask Seer Tech Data', iconClass: 'oj-fwk-icon oj-fwk-icon-magnifier', featureTags: ['Natural Language SQL', 'Live Schema'] },
  { id: 'agents', label: 'AI Agent Console', iconClass: 'oj-fwk-icon oj-fwk-icon-users', featureTags: ['PL/SQL Tools', 'JSON Audit'] },
];

function IdentityScopedPage({ activePage, onNavigate }) {
  const { currentUser } = useUser();
  const identityKey = currentUser?.USERNAME || 'viewer_sam';

  if (activePage === 'welcome') {
    return <Welcome key={`welcome:${currentUser?.USERNAME || identityKey}`} onNavigate={onNavigate} />;
  }

  const PageComponent = PAGES[activePage];
  if (!PageComponent) return null;
  const pageProps = activePage === 'datamodel' ? { onNavigate } : {};
  return <PageComponent key={`${activePage}:${currentUser?.USERNAME || identityKey}`} {...pageProps} />;
}
...
function deriveShortName(customerName) {
  ...
  return words.slice(0, 2).join(' ') || CUSTOMER_SHORT_NAME;
}
...
function replaceCustomerTerms(value, profile) {
  ...
  let next = value.replace(/\bseer-tech\.example\b/gi, profile.customerDomain);
  if (profile.isDefault) {
    next = next.replace(/\bSeer Tech Product Intelligence LiveStack\b/g, `${CUSTOMER_NAME} Product Intelligence LiveStack`);
    next = next.replace(new RegExp(`\\b${escapeRegExp(LEGACY_HIGH_TECH_PRODUCT_INTELLIGENCE)}\\b`, 'g'), `${CUSTOMER_NAME} Product Intelligence`);
    next = next.replace(/\bHigh Tech Demo Data\b/g, `${CUSTOMER_NAME} Demo Data`);
    next = next.replace(/\bSeer Tech demo data\b/g, `${CUSTOMER_NAME} demo data`);
    return next;
  }
  for (const alias of CUSTOMER_ALIASES) {
    next = next.replace(new RegExp(`\\b${escapeRegExp(alias)}\\b`, 'g'), profile.customerName);
  }
  next = next.replace(/\bseer-tech\.example\b/gi, profile.customerDomain);
  return next;
}

function hasReplaceableCustomerTerms(value) {
  return typeof value === 'string' && (
    /\bseer-tech\.example\b/i.test(value) ||
    /\bHigh Tech(?: Product Intelligence| Demo Data| product intelligence)?\b/.test(value) ||
    /\bSeer Tech demo data\b/.test(value) ||
    /\bSeer(?: Equity Bank| Equity| Bank)?\b/.test(value)
  );
}
...
function OracleBrand({ profile }) {
  const title = replaceCustomerTerms('Seer Tech Product Intelligence LiveStack', profile);
  return (
    <button
      type="button"
...
function FeatureTagList({ tags, variant = 'default' }) {
  ...
  return (
    <span className={`feature-tag-list feature-tag-list--${variant}`} aria-label="Oracle AI Database 26ai features">
      {tags.map((tag) => (
    ...
    <div className="customer-name-tool">
      ...
      {isOpen && (
        <form className="customer-name-tool__panel" onSubmit={submit}>
          ...
          <p className="customer-name-tool__hint">Replaces visible demo customer labels while leaving SQL snippets, JSON keys, KPI values, and data rows unchanged.</p>
        </form>
      )}
    </div>
  );
          ...
          <div className="app-shell">
            ...
            <div className="app-main">
              <header className="app-topbar">
                ...
                <JetButton
                  label="Use Your Own Product Data"
              ...
              <main className="app-content">
                <div className="app-page-frame">
                  <IdentityScopedPage activePage={activePage} onNavigate={setActivePage} />
                </div>
              </main>
            </div>
