...
const AGENT_NAME_LABELS = {
  trend_detection_agent: 'Demand Volatility Agent',
  inventory_agent: 'Component Capacity Agent',
  fulfillment_agent: 'Order Promising Agent',
  master_orchestrator: 'Product & Commitment Orchestrator',
  chat_agent: 'High Tech Data Assistant',
};
...
const TOOL_NAME_LABELS = {
  ...
  TREND_SQL_TOOL: 'Demand Signal SQL',
  DETECT_TRENDS_TOOL: 'Demand Volatility Detector',
  INFLUENCER_NETWORK_TOOL: 'Product Graph Traversal',
  COMMERCE_SQL_TOOL: 'Commitment SQL Workbench',
  CHECK_INVENTORY_TOOL: 'BOM and Component Capacity Check',
  FULFILLMENT_ROUTE_TOOL: 'Order Promising Route Check',
  LOG_DECISION_TOOL: 'Governed Action Logger',
...
const ENTITY_TYPE_LABELS = {
  product: 'High Tech Product',
  order: 'Customer Commitment',
  inventory: 'Component Capacity',
  fulfillment_center: 'Supply Site',
  social_post: 'Product Signal',
  signal: 'Demand / Quality Signal',
};

const COLUMN_LABELS = {
  PRODUCT_NAME: 'High Tech Product',
  CENTER_NAME: 'Supply Site',
  QUANTITY_ON_HAND: 'Available Capacity',
  REORDER_POINT: 'Reorder / BOM Threshold',
  ORDER_TOTAL: 'Commitment Value',
  ORDER_STATUS: 'Order Promising Status',
};
...
function normalizeToolLabel(value) {
  ...
  return TOOL_NAME_LABELS[raw] || TOOL_NAME_LABELS[base] ||
...
function labelFor(map, value) {
  ...
  const raw = String(value).trim();
  const base = raw.replace(/\s*\([^)]*\)\s*$/, '');
  if (map === TOOL_NAME_LABELS) {
    return normalizeToolLabel(raw);
  }
  return map[raw] || map[base] || base.replace(/_/g, ' ').replace(/\b\w/g, (char) => char.toUpperCase());
}

const TEAM_INFO = {
  TECH_SIGNAL_AGENT: { label: 'Signal Agent', color: '#AA643B', iconClass: 'oj-fwk-icon-sortrelevancehigh', desc: 'Enterprise Buyer and community signal analysis' },
  PRODUCT_AVAILABILITY_AGENT:  { label: 'Product Availability Agent', color: '#437C94', iconClass: 'oj-fwk-icon-tree-document', desc: 'Capacity and fulfillment-site routing' },
  PRODUCT_OPERATIONS_AGENT:     { label: 'Product Operations Agent', color: '#4C825C', iconClass: 'oj-fwk-icon-grid', desc: 'Customer commitments and commitment value' },
};

const EXAMPLE_QUESTIONS = [
  { text: 'Which high-tech products are seeing the highest demand right now?', iconClass: 'oj-fwk-icon-sortrelevancehigh', team: 'trends' },
  { text: 'Show me product value by product category', iconClass: 'oj-fwk-icon-grid', team: 'product operations' },
  { text: 'Which high-tech products have low capacity?', iconClass: 'oj-fwk-icon-tree-document', team: 'fulfillment' },
  { text: 'What percentage of customer commitments are product-signal-driven?', iconClass: 'oj-fwk-icon-info', team: 'product operations' },
  { text: 'Find urgent product-signal posts in the last 24 hours', iconClass: 'oj-fwk-icon-sortrelevancehigh', team: 'trends' },
  { text: 'Check capacity for Cloud GPU Burst Reservation', iconClass: 'oj-fwk-icon-tree-document', team: 'fulfillment' },
  { text: 'Find the nearest availability center for an enterprise buyer in Miami', iconClass: 'oj-fwk-icon-arrowtail-e', team: 'fulfillment' },
];

const AGENT_CONVERSATION_STORAGE_KEY = 'hightech.agent-console.conversation.v1';
const MAX_PERSISTED_AGENT_MESSAGES = 24;
...
function loadPersistedAgentMessages() {
  if (typeof window === 'undefined') return [];
  try {
    const saved = JSON.parse(window.sessionStorage.getItem(AGENT_CONVERSATION_STORAGE_KEY) || '[]');
    return Array.isArray(saved)
      ? saved.filter((message) => message && ['user', 'agent'].includes(message.role)).slice(-MAX_PERSISTED_AGENT_MESSAGES)
      : [];
  } catch (_) {
    return [];
  }
...
// ── Product Availability Route Map (rendered inside chat messages) ─────────────────────
function ProductAvailabilityRouteMap({ routeData }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-[var(--color-border)]" style={{ background: 'var(--color-surface)' }}>
      ...
      <div className="px-3 py-2 flex items-center gap-2" style={{ background: 'rgba(67,124,148,0.08)', borderBottom: '1px solid rgba(67,124,148,0.2)' }}>
        ...
        <span className="text-xs font-bold tone-ocean">Product Availability Route - {product}</span>
      ...
      <div style={{ height: 240 }}>
        <MapContainer bounds={bounds} style={{ height: '100%', width: '100%', borderRadius: 0 }}
          ...
          {/* Product Availability center markers */}
          {centers.map((c, i) => (
            <CircleMarker key={i}
              center={[c.lat, c.lon]}
              radius={i === 0 ? 8 : 5}
          ...
          <CircleMarker center={[customer.lat, customer.lon]} radius={7}
            ...
            <MapTooltip permanent direction="bottom" offset={[0, 8]}
              ...
              <div style={{ fontSize: 10 }}>
                <strong>Enterprise Buyer</strong><br />{customer.city}, {customer.state}
              </div>
            </MapTooltip>
          </CircleMarker>
        </MapContainer>
...
function ChatAgent({ onActionLogged }) {
  const [messages, setMessages] = useState(loadPersistedAgentMessages);
  ...
  useEffect(() => {
    ...
    try {
      ...
      window.sessionStorage.setItem(AGENT_CONVERSATION_STORAGE_KEY, JSON.stringify(persistedMessages));
    } catch (_) {
      // A full browser session store must not prevent the agent console from working.
    }
  }, [messages]);
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      const result = await api.agents.chat(question, buildAgentConversationHistory(messages));
  ...
  const clearChat = useCallback(() => {
    ...
    if (typeof window !== 'undefined') {
      window.sessionStorage.removeItem(AGENT_CONVERSATION_STORAGE_KEY);
    }
  }, []);
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-[var(--color-text-dim)]">
              Ask questions - auto-routed to <span className="font-semibold text-[var(--color-text)]">Signal</span>, <span className="font-semibold text-[var(--color-text)]">Product Availability</span>, or <span className="font-semibold text-[var(--color-text)]">Product Operations</span> agents
            </p>
          </div>
        </div>
        {messages.length > 0 && (
        ...
        {messages.length === 0 && (
          <div className="space-y-3 py-4">
            <div className="text-center mb-4">
              ...
              <p className="text-sm text-[var(--color-text-dim)]">Ask me anything about your high-tech product intelligence data</p>
              ...
              {msg.role === 'agent' && (
                <div className="space-y-2">
                  ...
                  {msg.data && msg.data.type === 'route' && (
                    <ProductAvailabilityRouteMap routeData={msg.data} />
                  )}
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {msg.toolsUsed.map((t, ti) => (
                        <span key={ti} className="text-[9px] px-2 py-0.5 rounded-full font-mono flex items-center gap-1"
                          ...
                          <JetGlyph iconClass="oj-fwk-icon-tree-document" /> {labelFor(TOOL_NAME_LABELS, t.tool)}
                        </span>
                      ))}
                    </div>
                  )}
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-[var(--color-text)] leading-relaxed">
              ...
              and Oracle AI Database 26ai executing SQL and PL/SQL tools against live data. Three specialist teams (<code className="text-xs font-semibold" style={{ color: 'var(--color-text)', borderBottom: '1px solid #AA643B' }}>TECH_SIGNAL_AGENT</code>,&nbsp;
              <code className="text-xs font-semibold" style={{ color: 'var(--color-text)', borderBottom: '1px solid #437C94' }}>PRODUCT_AVAILABILITY_AGENT</code>, <code className="text-xs font-semibold" style={{ color: 'var(--color-text)', borderBottom: '1px solid #4C825C' }}>PRODUCT_OPERATIONS_AGENT</code>) route work across
              supply planning, manufacturing capacity, quality review, customer operations, and product operations tasks. Oracle stores the source data, runs the queries, and records decisions in
...
-- Example flow:
-- 1. Classify the request as TECH_SIGNAL_AGENT
  ...
  :product_id, :json_payload, 0.92, 'proposed');`} />
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { team: 'TECH_SIGNAL_AGENT', agent: 'TECH_SIGNAL_AGENT', tools: ['TREND_SQL_TOOL', 'DETECT_TRENDS_TOOL', 'INFLUENCER_NETWORK_TOOL', 'LOG_DECISION_TOOL'], color: '#A36472' },
                { team: 'PRODUCT_AVAILABILITY_AGENT', agent: 'PRODUCT_AVAILABILITY_AGENT', tools: ['COMMERCE_SQL_TOOL', 'CHECK_INVENTORY_TOOL', 'FULFILLMENT_ROUTE_TOOL', 'LOG_DECISION_TOOL'], color: '#AA643B' },
                { team: 'PRODUCT_OPERATIONS_AGENT', agent: 'PRODUCT_OPERATIONS_AGENT', tools: ['COMMERCE_SQL_TOOL', 'LOG_DECISION_TOOL'], color: '#4C825C' },
              ].map(t => (
                    ...
                    <div className="space-y-0.5">
                      {t.tools.map(tool => (
                        <div key={tool} className="text-[8px] font-mono px-1.5 py-0.5 rounded bg-[var(--color-surface)] text-[var(--color-text-dim)]">
                          {labelFor(TOOL_NAME_LABELS, tool)}
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <div>
            ...
            <div className="space-y-1" style={{ fontSize: 9 }}>
              <DiagramBox label="Urgent Enterprise Buyer Signal Detected" sub="social_posts · virality_score >= 75" color="#AA643B" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="TECH_SIGNAL_AGENT" sub="Ollama reasoning + tool routing" color="#796087" />
              ...
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="PRODUCT_AVAILABILITY_AGENT" sub="Capacity check · reorder logic" color="#437C94" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="PRODUCT_OPERATIONS_AGENT" sub="Pricing · promotions · routing" color="#4C825C" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
      ...
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <JetGlyph iconClass="oj-fwk-icon-users" className="agent-console-page-glyph tone-plum" /> Agent Orchestration Console
          </h2>
          <p className="text-sm text-[var(--color-text-dim)] mt-1">
            Agent teams use <span className="font-semibold text-[var(--color-text)]">Ollama</span> for reasoning and Oracle for data, SQL execution, and action logging
          </p>
        </div>
        <div className="flex items-center gap-2">
          {/* AI Profile Selector */}
              ...
              <div key={a.ACTION_ID} className="flex items-start gap-3 p-3 rounded-lg hover:bg-[var(--color-surface-hover)] transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {a.ENTITY_TYPE && (
                      <span className="text-[10px] text-[var(--color-text-dim)]">{labelFor(ENTITY_TYPE_LABELS, a.ENTITY_TYPE)} #{a.ENTITY_ID}</span>
                    )}
                  </div>
                  {payload && (
