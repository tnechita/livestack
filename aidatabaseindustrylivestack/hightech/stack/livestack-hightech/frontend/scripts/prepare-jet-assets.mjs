...
import { readdir, readFile, writeFile as writeFileAsync } from 'node:fs/promises';
...
async function sanitizePlainPunctuation(targetDir) {
  const entries = await readdir(targetDir, { withFileTypes: true });
  await Promise.all(entries.map(async (entry) => {
    const entryPath = path.join(targetDir, entry.name);
    if (entry.isDirectory()) {
      await sanitizePlainPunctuation(entryPath);
      return;
    }
    ...
    if (!original.includes(EM_DASH) && !original.includes('&mdash;')) return;
    const sanitized = original.replace(new RegExp(EM_DASH, 'g'), '-').replace(/&mdash;/g, '-');
    await writeFileAsync(entryPath, sanitized, 'utf8');
  }));
}
...
await sanitizePlainPunctuation(jetDir);
