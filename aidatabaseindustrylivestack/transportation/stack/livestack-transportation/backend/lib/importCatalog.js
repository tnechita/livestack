  ...
  },
  {
    ...
    columns: [
      idColumn('order_id'),
        ...
        stringColumn('cancellation_reason'),
...
function getTable(tableOrName) {
  if (!tableOrName) return null;
  return typeof tableOrName === 'string' ? TABLE_BY_NAME[tableOrName] : tableOrName;
}

function getTableDisplayName(tableOrName) {
  const table = getTable(tableOrName);
  return table?.displayName || table?.name || String(tableOrName || '');
}

function getTableCsvFileName(tableOrName) {
  const table = getTable(tableOrName);
  return table?.fileName || `${table?.name || tableOrName}.csv`;
}

function getTableAcceptedCsvFileNames(tableOrName) {
  const table = getTable(tableOrName);
  if (!table) return [];
  return [...new Set([getTableCsvFileName(table), `${table.name}.csv`, ...(table.legacyFileNames || [])])];
}
...
function buildManifest() {
  return {
    ...
    format: 'transportation-fleet-logistics-import',
    exportedAt: new Date().toISOString(),
    notes: TEMPLATE_NOTES,
    requiredTables: REQUIRED_TABLE_NAMES,
    optionalTables: OPTIONAL_TABLE_NAMES,
    requiredFiles: TABLES.filter((table) => table.required).map((table) => getTableCsvFileName(table)),
    optionalFiles: TABLES.filter((table) => !table.required).map((table) => getTableCsvFileName(table)),
    tables: TABLES.map((table) => ({
      name: table.name,
      displayName: getTableDisplayName(table),
      file: getTableCsvFileName(table),
