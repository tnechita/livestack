 ...
 *   CUSTOMER_SEGMENT_MODEL  — K-Means clustering for shipper health
 *   REVENUE_PREDICT_MODEL   — GLM regression for shipment service value
 *   PRODUCT_CLUSTER_MODEL   — K-Means clustering for service grouping by behavior
 *
...
router.get('/models/status', async (req, res) => {
  try {
    const result = await db.executeAsUser(`SELECT model_name, mining_function, algorithm
      ...
      ORDER BY model_name`, {}, req.demoUser);
    const active = new Set(result.rows.map((row) => row.MODEL_NAME));
    const models = EXPECTED_OML_MODELS.map((modelName) => ({
      modelName,
      active: active.has(modelName),
    }));
    const ready = models.every((model) => model.active);
    return res.status(ready ? 200 : 503).json({
      ready,
      source: 'USER_MINING_MODELS',
      expectedModels: EXPECTED_OML_MODELS,
      activeCount: models.filter((model) => model.active).length,
      models,
    });
  } catch (err) {
...
router.get('/persistence/status', async (req, res) => {
  try {
    const result = await db.executeAsUser(`SELECT COUNT(*) AS active_count
      FROM user_mining_models
      WHERE model_name IN (
        'DEMAND_SURGE_MODEL',
        'CUSTOMER_SEGMENT_MODEL',
    ...
    const persistent = activeCount === EXPECTED_OML_MODELS.length;
    return res.status(persistent ? 200 : 503).json({
      persistent,
      modelLifecycle: { ready: persistent },
      activeCount,
      source: 'USER_MINING_MODELS',
...
async function handleMlRouteError(res, label, err, fallbackFn) {
  if (isMissingMlAssetError(err)) {
    console.error(`${label} persisted OML asset is not ready:`, err);
    return res.status(503).json({
      error: 'Required persisted Oracle Machine Learning model is not ready.',
      code: 'OML_MODEL_NOT_READY',
...
function buildDemandProductFeaturesCte(lookbackHoursExpr = null) {
  const socialWhere = lookbackHoursExpr
    ? `WHERE CAST(sp.POSTED_AT AS DATE) >= ${SOCIAL_POSTED_AT_ANCHOR_SQL} - ${lookbackHoursExpr} / 24`
    : '';
  const salesWhere = lookbackHoursExpr
    ? `WHERE CAST(o.CREATED_AT AS DATE) >= ${ORDER_CREATED_AT_ANCHOR_SQL} - ${lookbackHoursExpr} / 24`
    : '';
...
async function fallbackDemandForecast({ limit, lookbackHours }) {
  ...
  return {
    ...
    meta: {
      ...
      scoring: 'Weighted operational signal + shipment request signal',
...
async function fallbackCustomerSegments({ limit }) {
  const result = await db.execute(`
    ...
    segmented AS (
      SELECT s.*,
             ...
             CASE
               WHEN s.RECENCY_SCORE = 4 AND s.FREQUENCY_SCORE >= 3 AND s.MONETARY_SCORE >= 3 THEN 'Strategic Account'
               WHEN s.RECENCY_SCORE >= 3 AND s.FREQUENCY_SCORE >= 3 THEN 'Reliable Shipper'
               WHEN s.RECENCY_SCORE = 4 AND s.FREQUENCY_SCORE <= 2 THEN 'New Shipper'
               WHEN s.RECENCY_SCORE <= 2 AND s.MONETARY_SCORE = 4 THEN 'SLA Risk'
               WHEN s.RECENCY_SCORE = 1 AND s.FREQUENCY_SCORE <= 2 THEN 'Dormant Shipper'
               WHEN s.MONETARY_SCORE = 4 AND s.RECENCY_SCORE >= 2 THEN 'High-Value Shipper'
               WHEN s.RECENCY_SCORE >= 3 AND s.MONETARY_SCORE <= 2 THEN 'Growth Lane'
               ELSE 'Developing Account'
             END AS SEGMENT,
             CASE
  ...
  return {
    ...
    commitmentRiskDistribution: Object.entries(churnDist).map(([risk, count]) => ({ risk, count })),
    total: result.rows.length,
    meta: {
      model: 'Shipper health segmentation heuristic (fallback)',
      algorithm: 'QUARTILE_SEGMENTATION',
      scoring: 'NTILE quartiles + deterministic shipper-health rules',
...
async function fallbackRevenueForecast({ lookbackDays, forecastDays }) {
  const histResult = await db.execute(`
    WITH daily_rev AS (
      ...
      FROM ORDERS
      WHERE CAST(CREATED_AT AS DATE) >= ${ORDER_CREATED_AT_ANCHOR_SQL} - :lookback
  ...
  const forecast = [];
  const lastHistoricalDay = new Date(`${last.DAY}T00:00:00Z`);
  for (let i = 1; i <= forecastDays; i++) {
    const futureRn = maxRn + i;
    const predicted = Math.max(0, slope * futureRn + intercept);
    const ci = stddev * (1 + i * 0.07);
    const d = Number.isNaN(lastHistoricalDay.getTime()) ? new Date() : new Date(lastHistoricalDay.getTime());
    ...
    const result = await db.execute(`
      WITH product_features AS (
        ...
        LEFT JOIN (
            ...
            FROM POST_PRODUCT_MENTIONS ppm
            JOIN SOCIAL_POSTS sp ON ppm.POST_ID = sp.POST_ID
            WHERE CAST(sp.POSTED_AT AS DATE) >= ${SOCIAL_POSTED_AT_ANCHOR_SQL} - :lookback / 24
            GROUP BY ppm.PRODUCT_ID
        ) eng ON p.PRODUCT_ID = eng.PRODUCT_ID
        LEFT JOIN (
            SELECT oi.PRODUCT_ID,
            ...
            FROM ORDER_ITEMS oi
            JOIN ORDERS o ON o.ORDER_ID = oi.ORDER_ID
            WHERE CAST(o.CREATED_AT AS DATE) >= ${ORDER_CREATED_AT_ANCHOR_SQL} - :lookback / 24
            GROUP BY oi.PRODUCT_ID
        ) sales ON p.PRODUCT_ID = sales.PRODUCT_ID
        WHERE p.IS_ACTIVE = 1
      )
      ...
      SELECT
        ...
        CASE
          WHEN NTILE(4) OVER (ORDER BY cm.recency_days ASC) = 4
           ...
           AND NTILE(4) OVER (ORDER BY cm.monetary DESC) >= 3 THEN 'Strategic Account'
          WHEN NTILE(4) OVER (ORDER BY cm.recency_days ASC) >= 3
           AND NTILE(4) OVER (ORDER BY cm.frequency DESC) >= 3 THEN 'Reliable Shipper'
          WHEN NTILE(4) OVER (ORDER BY cm.recency_days ASC) = 4
           AND NTILE(4) OVER (ORDER BY cm.frequency DESC) <= 2 THEN 'New Shipper'
          WHEN NTILE(4) OVER (ORDER BY cm.recency_days ASC) <= 2
           AND NTILE(4) OVER (ORDER BY cm.monetary DESC) = 4 THEN 'SLA Risk'
          WHEN NTILE(4) OVER (ORDER BY cm.recency_days ASC) = 1
           AND NTILE(4) OVER (ORDER BY cm.frequency DESC) <= 2 THEN 'Dormant Shipper'
          WHEN NTILE(4) OVER (ORDER BY cm.monetary DESC) = 4
           AND NTILE(4) OVER (ORDER BY cm.recency_days ASC) >= 2 THEN 'High-Value Shipper'
          WHEN NTILE(4) OVER (ORDER BY cm.recency_days ASC) >= 3
           AND NTILE(4) OVER (ORDER BY cm.monetary DESC) <= 2 THEN 'Growth Lane'
          ELSE 'Developing Account'
        END AS segment,
    ...
    const [surgeCount, customers, modelCount, regr] = await Promise.all([
      ...
      db.execute(`
        SELECT COUNT(*) AS cnt FROM (
          ...
          LEFT JOIN (
              ...
              FROM POST_PRODUCT_MENTIONS ppm
              JOIN SOCIAL_POSTS sp ON ppm.POST_ID = sp.POST_ID
              WHERE CAST(sp.POSTED_AT AS DATE) >= ${SOCIAL_POSTED_AT_ANCHOR_SQL} - 30
              GROUP BY ppm.PRODUCT_ID
          ) eng ON p.PRODUCT_ID = eng.PRODUCT_ID
          LEFT JOIN (
      ...
      db.execute(`SELECT /*+ NO_PARALLEL */
        ...
        FROM (
          SELECT SUM(order_total) AS day_rev,
                 ROW_NUMBER() OVER (ORDER BY TRUNC(CAST(created_at AS DATE))) AS rn
          FROM orders WHERE CAST(created_at AS DATE) >= ${ORDER_CREATED_AT_ANCHOR_SQL} - 30
          GROUP BY TRUNC(CAST(created_at AS DATE))
        )`),
    ]);
...
//
// Scores the lifecycle-owned PRODUCT_CLUSTER_MODEL without mutating it.
// ─────────────────────────────────────────────────────────────────────────────
router.get('/vector-clusters', async (req, res) => {
  try {
    ...
    res.json({
      ...
      clusters,
      meta: {
        model: `PRODUCT_CLUSTER_MODEL (persisted K-Means, ${clusters.length} active clusters)`,
        algorithm: 'ALGO_KMEANS',
        scoring: 'CLUSTER_ID() / CLUSTER_PROBABILITY()',
