...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await db.executeAsUser(`
      SELECT o.order_id, o.order_status, o.cancellation_reason, o.order_total, o.shipping_cost,
...
router.get('/:id/duality', async (req, res) => {
  try {
    ...
    if (!Number.isSafeInteger(orderId) || orderId < 1) {
      return res.status(400).json({ error: 'A positive order ID is required.' });
    }
    ...
    res.status(503).json({
      error: 'Required ORDERS_DV JSON Relational Duality View is unavailable.',
