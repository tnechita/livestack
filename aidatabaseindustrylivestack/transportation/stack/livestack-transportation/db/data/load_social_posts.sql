 ...
 * 5000 service/disruption signal posts with realistic transportation text and service mentions
 */

SET SERVEROUTPUT ON
PROMPT Loading service and disruption signal posts...
...
DECLARE
    ...
    v_templates t_str := t_str(
        'A lot of shippers are asking about {product} through {brand}; lane demand is clearly rising',
        'Dispatch coordinators are flagging {product} from {brand} as a bottleneck this week',
        'Terminal update: {product} capacity at {brand} is getting tight after a spike in tenders',
        'Shipper operations note - {brand} {product} is the service everyone is trying to schedule right now',
        'Telematics follow-up: demand for {product} is up and {brand} needs more slots',
        'Two-week review of {brand} {product}: strong service levels, but capacity planning matters',
        'Accounts keep asking where to find {product}. {brand} is showing up in every routing thread',
        'Control tower huddle: prioritize {brand} {product} tenders before the weekend surge',
        'Port and weather alerts are surfacing new need for {product} from {brand}',
        'If your shippers need {product}, check {brand} availability early; slots are moving fast',
        'Day 30 with the {product} workflow and dispatch says {brand} reduced manual follow-up',
        'Recommended {brand} {product} to a load planner today because missed pickup windows are the risk',
        'Thought {product} demand would level off, but {brand} is still seeing urgent requests',
        'Morning network review featuring {product}. {brand} needs pre-positioned fleet capacity',
        'Added {product} to the high-priority route plan. Thank you {brand} for closing the gap'
    );

    -- Additional organic-sounding transportation posts (no service line mention)
    v_generic t_str := t_str(
        'Shippers are asking for clearer pickup instructions and faster appointment windows',
        'Final-mile capacity is the top request in our shipper operations forum this week',
        'Dispatchers need earlier visibility into terminal appointment availability',
        'Remote trailer alerts are helping the team catch dwell risk before a missed delivery',
        'Weather and port congestion are delaying service commitments for several shipper groups',
        'The terminal is seeing increased demand for linehaul capacity after a national shipper launch',
        'Shippers keep asking for tighter ETA updates before weekend deliveries',
        'Safety teams need better exception outreach after recent route disruptions',
        'Network planners are coordinating tenders across regional terminals today',
        'Driver availability screening is creating new handoffs to carrier partners'
    );
...
BEGIN
    ...
    FOR i IN 1..5000 LOOP
        ...
        -- 70% service-line mention posts, 30% generic
        ...
        INSERT INTO social_posts (
            ...
            v_sentiment,
            ROUND(LEAST(100,
                LEAST(v_likes / 1000, 35) +
                LEAST(v_shares / 250, 25) +
                LEAST(v_comments / 200, 15) +
                LEAST(v_views / 100000, 25)
            ), 2),
            CASE
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Shipper/community signal posts loaded: ' || v_count);
END;
/
