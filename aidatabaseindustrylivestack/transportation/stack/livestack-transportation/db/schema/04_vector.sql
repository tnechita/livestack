/*
 * 04_vector.sql
 * Vector Search for semantic product matching from signal bulletins
 * Oracle 26ai — native VECTOR type and similarity search
 *
 * Embedding model : all_MiniLM_L12_v2 (384 dimensions, cosine)
 * Source          : deployment-provided ONNX artifact copied to DATA_PUMP_DIR
 */
...
--
--   Obtain the ONNX artifact from an approved deployment-local source.
--   Upload it to DATA_PUMP_DIR via ONE of:
--     A) Database Actions ▶ Data Studio ▶ Files  (browser upload)
--     B) OCI CLI or DBMS_CLOUD.GET_OBJECT with deployment-local credentials
--
...
-- ============================================================
-- PRODUCT EMBEDDINGS
-- Pre-computed vector embeddings for transportation service descriptions
-- Used to semantically match signal bulletin text -> products
-- ============================================================
...
-- Stores computed matches between signal bulletins and products
-- ============================================================
...
-- Find products semantically similar to a signal bulletin
...
AS
    v_post_vector VECTOR(384, FLOAT32);
BEGIN
...
-- Batch process: find products for all unprocessed signal bulletins
...
AS
    v_query_vec VECTOR(384, FLOAT32);
BEGIN
