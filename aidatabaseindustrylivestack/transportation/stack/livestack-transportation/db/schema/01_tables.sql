...
-- ============================================================
-- FULFILLMENT CENTERS (logistics terminals / service hubs kept on baseline table name)
-- ============================================================
...
-- ============================================================
-- INVENTORY (transportation service capacity / transport equipment supply units at each access center)
-- ============================================================
...
CREATE TABLE orders (
    ...
    social_source_id   NUMBER,         -- FK to social_posts if transport order was influenced by a service/disruption signal
...
-- Durable singleton lease. Dataset Restore workers must hold this Oracle row;
-- a process-local mutex is not authoritative across application restart.
CREATE TABLE app_dataset_operation_lock (
    lock_id          NUMBER(1) PRIMARY KEY CHECK (lock_id = 1),
    lease_token      VARCHAR2(100),
    owner_job_id     VARCHAR2(80),
...
INSERT INTO app_dataset_operation_lock (lock_id, updated_at)
...
CREATE TABLE app_dataset_jobs (
    ...
    progress            NUMBER(3) NOT NULL CHECK (progress BETWEEN 0 AND 100),
...
CREATE INDEX idx_app_dataset_jobs_status
ON app_dataset_jobs(status, updated_at);
