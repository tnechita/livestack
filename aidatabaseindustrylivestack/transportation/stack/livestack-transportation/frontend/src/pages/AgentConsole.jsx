...
const AGENT_COLORS = {
  ...
  signal_detection_agent: '#AA643B',
  capacity_agent: '#437C94',
...
const AGENT_DISPLAY_NAMES = {
  trend_detection_agent: 'Signal Agent',
  signal_detection_agent: 'Signal Agent',
  capacity_agent: 'Terminal Access Agent',
  inventory_agent: 'Terminal Access Agent',
  fulfillment_agent: 'Terminal Access Agent',
  master_orchestrator: 'Fleet Operations Agent',
  chat_agent: 'AI Agent Router',
};

const TEAM_INFO = {
  TRK_SIGNAL_TEAM: { label: 'Signal Agent', color: '#AA643B', iconClass: 'oj-fwk-icon-sortrelevancehigh', desc: 'Operational disruption and demand signal analysis' },
  TRK_ACCESS_TEAM:  { label: 'Terminal Access Agent', color: '#437C94', iconClass: 'oj-fwk-icon-tree-document', desc: 'Capacity and terminal routing' },
  TRK_OPERATIONS_TEAM:     { label: 'Shipment Operations Agent', color: '#4C825C', iconClass: 'oj-fwk-icon-grid', desc: 'Shipment orders and freight value' },
};

const EXAMPLE_QUESTIONS = [
  { text: 'Which services are at highest surge risk on the Northeast Corridor?', iconClass: 'oj-fwk-icon-sortrelevancehigh', team: 'TRK_SIGNAL_TEAM' },
  { text: 'Show freight value at risk by transport service category', iconClass: 'oj-fwk-icon-grid', team: 'TRK_OPERATIONS_TEAM' },
  { text: 'Which terminals can absorb urgent shipment orders this week?', iconClass: 'oj-fwk-icon-tree-document', team: 'TRK_ACCESS_TEAM' },
  { text: 'What percentage of shipment orders are operational-signal-driven?', iconClass: 'oj-fwk-icon-info', team: 'TRK_OPERATIONS_TEAM' },
  { text: 'Find critical port, weather, and shipper signals in the last 24 hours', iconClass: 'oj-fwk-icon-sortrelevancehigh', team: 'TRK_SIGNAL_TEAM' },
  { text: 'Check capacity for Regional LTL Freight Move', iconClass: 'oj-fwk-icon-tree-document', team: 'TRK_ACCESS_TEAM' },
  { text: 'Find nearest terminal with same-day slots for a shipper in Miami', iconClass: 'oj-fwk-icon-arrowtail-e', team: 'TRK_ACCESS_TEAM' },
];
...
// ── Terminal Access Route Map (rendered inside chat messages) ─────────────────────
function FulfillmentRouteMap({ routeData }) {
  const { shipper, centers, product } = routeData;
  if (!shipper || !centers || centers.length === 0) return null;

  // Calculate map bounds
  const allPoints = [
    [shipper.lat, shipper.lon],
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-[var(--color-border)]" style={{ background: 'var(--color-surface)' }}>
      ...
      <div className="px-3 py-2 flex items-center gap-2" style={{ background: 'rgba(67,124,148,0.08)', borderBottom: '1px solid rgba(67,124,148,0.2)' }}>
        ...
        <span className="text-xs font-bold tone-ocean">Terminal Access Route - {product}</span>
      ...
      <div style={{ height: 240 }}>
        <MapContainer bounds={bounds} style={{ height: '100%', width: '100%', borderRadius: 0 }}
          ...
          {/* Route lines from shipper to each center */}
          {centers.map((c, i) => (
            <Polyline key={i}
              positions={[[shipper.lat, shipper.lon], [c.lat, c.lon]]}
              pathOptions={{
...
function ChatAgent({ onActionLogged }) {
  ...
  const buildConversationHistory = useCallback(() => messages
    ...
    .filter((message) => message.text), [messages]);
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      const result = await api.agents.chat(question, buildConversationHistory());
  ...
  }, [input, sending, onActionLogged, buildConversationHistory]);
    ...
    <div className="glass-card overflow-hidden" style={{ border: '1px solid rgba(79,125,123,0.25)' }}>
      ...
      <div className="px-5 py-3 flex items-center justify-between" style={{ background: 'rgba(79,125,123,0.06)', borderBottom: '1px solid rgba(79,125,123,0.15)' }}>
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold">Coordinate the recovery decision</h3>
            <p className="text-[10px] text-[var(--color-text-dim)]">
              Ask questions - auto-routed to <span className="font-semibold text-[var(--color-text)]">Signal</span>, <span className="font-semibold text-[var(--color-text)]">Terminal Access</span>, or <span className="font-semibold text-[var(--color-text)]">Shipment Operations</span> agents
            </p>
          </div>
        </div>
        {messages.length > 0 && (
        ...
        {messages.length === 0 && (
          <div className="space-y-3 py-4">
            <div className="text-center mb-4">
              ...
              <p className="text-sm text-[var(--color-text-dim)]">Ask how to protect the shipment commitment, find capacity, or explain the disruption.</p>
              ...
              {EXAMPLE_QUESTIONS.map((eq, i) => (
                <div key={i} className="agent-console-example-tile">
                  <div className="agent-console-example-meta">
                    ...
                    <span className="text-[9px] text-[var(--color-text-dim)] uppercase">{TEAM_INFO[eq.team]?.label || eq.team}</span>
                  </div>
...
export default function AgentConsole() {
  ...
  return (
    <div className="space-y-6 fade-in">
      <TransportationStoryPanel scene="agents" />

      {/* Register Oracle Internals into the right panel */}
      <RegisterOraclePanel title="Operations Agent Console">
        <div className="space-y-4">
          <div>
            ...
            <p className="text-[var(--color-text)] leading-relaxed">
              ...
              and Oracle AI Database 26ai executing SQL and PL/SQL tools against live data. Three specialist teams (<code className="text-xs font-semibold" style={{ color: 'var(--color-text)', borderBottom: '1px solid #AA643B' }}>TRK_SIGNAL_TEAM</code>,&nbsp;
              <code className="text-xs font-semibold" style={{ color: 'var(--color-text)', borderBottom: '1px solid #437C94' }}>TRK_ACCESS_TEAM</code>, <code className="text-xs font-semibold" style={{ color: 'var(--color-text)', borderBottom: '1px solid #4C825C' }}>TRK_OPERATIONS_TEAM</code>) route work across
              signal analysis, terminal access, and shipment operations tasks. Oracle stores the source data, runs the queries, and records decisions in
...
-- Example flow:
-- 1. Classify the request as TRK_SIGNAL_TEAM
-- 2. Call detect_trending_products(p_hours=>24) as the operational signal detector
-- 3. Join capacity and logistics terminal data in Oracle
...
VALUES ('signal_detection_agent','capacity_intervention','transport_service',
  :transport_service_id, :json_payload, 0.92, 'proposed');`} />
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { team: 'TRK_SIGNAL_TEAM', agent: 'Signal Agent', tools: ['SIGNAL_SQL_TOOL', 'DETECT_SIGNALS_TOOL', 'TRANSPORT_NETWORK_TOOL', 'LOG_DECISION_TOOL'], color: '#A36472' },
                { team: 'TRK_ACCESS_TEAM', agent: 'Terminal Access Agent', tools: ['CAPACITY_SQL_TOOL', 'CHECK_CAPACITY_TOOL', 'TERMINAL_ROUTE_TOOL', 'LOG_DECISION_TOOL'], color: '#AA643B' },
                { team: 'TRK_OPERATIONS_TEAM', agent: 'Shipment Operations Agent', tools: ['ORDER_SQL_TOOL', 'SERVICE_VALUE_TOOL', 'LOG_DECISION_TOOL'], color: '#4C825C' },
              ].map(t => (
          ...
          <div>
            ...
            <div className="space-y-1" style={{ fontSize: 9 }}>
              <DiagramBox label="Critical Operational Signal Detected" sub="shipper signal · urgency_score >= 75" color="#AA643B" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="TRK_SIGNAL_TEAM" sub="Ollama reasoning + tool routing" color="#796087" />
              <div className="text-center text-[var(--color-text-dim)]">↓ calls PL/SQL tool</div>
              <DiagramBox label="Transport signal detector" sub="Vector match · Graph centrality" color="#A36472" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="TRK_ACCESS_TEAM" sub="Terminal capacity check · pre-positioning logic" color="#437C94" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="TRK_OPERATIONS_TEAM" sub="Service value · order attribution · routing" color="#4C825C" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
      ...
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <JetGlyph iconClass="oj-fwk-icon-users" className="agent-console-page-glyph tone-plum" /> Operations Agent Console
          </h2>
          <p className="text-sm text-[var(--color-text-dim)] mt-1">
            Detect the risk, recommend a capacity action, explain the evidence, and log the decision.
          </p>
        </div>
        <div className="flex items-center gap-2">
          {/* AI Profile Selector */}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-1.5 py-0.5 rounded text-[9px] font-medium"
                      ...
                      {AGENT_DISPLAY_NAMES[a.AGENT_NAME] || a.AGENT_NAME.replace(/_/g, ' ')}
                    </span>
                    {a.ENTITY_TYPE && (
