...
const SEGMENT_COLORS = {
  'Strategic Account': '#AA643B',
  'Reliable Shipper': '#4C825C',
  'New Shipper': '#4F7D7B',
  'SLA Risk': '#C74634',
  'Dormant Shipper': '#7A736E',
  'High-Value Shipper': '#796087',
  'Growth Lane': '#437C94',
  'Developing Account': '#6F757E',
};
...
const TABS = [
  { key: 'demand',    label: 'Transport Demand Predictions',             buttonLabel: 'Demand Surge',     iconClass: 'oj-fwk-icon-sortrelevancehigh', color: '#AA643B' },
  { key: 'rfm',       label: 'Shipper Health Segmentation',            buttonLabel: 'Shipper Health',      iconClass: 'oj-fwk-icon-users',             color: '#C74634' },
  { key: 'forecast',  label: 'Freight Value Forecast - Linear Regression', buttonLabel: 'Forecast',          iconClass: 'oj-fwk-icon-view',              color: '#4C825C' },
  { key: 'clusters',  label: 'Service Similarity Clusters',            buttonLabel: 'Service Clusters',    iconClass: 'oj-fwk-icon-grid',              color: '#4F7D7B' },
  { key: 'capacity', label: 'Capacity Intelligence',               buttonLabel: 'Capacity',         iconClass: 'oj-fwk-icon-tree-document',     color: '#796087' },
];
...
function MomentumBadge({ flag }) {
  const label = flag === 'mega_urgent' ? 'MEGA' : flag?.replace('_', ' ') || '-';
  return (
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-[var(--color-text)] leading-relaxed">
          ...
          <code className="text-xs tone-sienna">DBMS_DATA_MINING.CREATE_MODEL</code> on 12 shipper-signal engagement
          and service-value features. Oracle scores every transportation service <em>inline</em> at query time using{' '}
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-[var(--color-text)] leading-relaxed">
          ...
          <code className="text-xs tone-plum">DBMS_DATA_MINING.CREATE_MODEL</code> on 6 shipper-health features.
          Each shipper account is assigned to a cluster using{' '}
          ...
          Shipper-health labels (Strategic Account, Reliable Shipper, SLA Risk, etc.) are layered on top via NTILE(4) window functions.
        </p>
      </div>
      <div className="flex flex-wrap gap-1.5">
...
ORDER BY total_spent DESC;`} />
      <div className="oml-model-flow">
        ...
        <DiagramBox label="Cluster Assignment + Probability" sub="each shipper account -> nearest centroid" color="#437C94" />
        <div className="text-center text-[10px] text-[var(--color-text-dim)]">↓ NTILE(4)</div>
        <DiagramBox label="Shipper Health Labels + Commitment Risk" sub="Strategic Account · Reliable Shipper · SLA Risk · Dormant Shipper" color="#4C825C" />
      </div>
    </div>
  );
}
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-[var(--color-text)] leading-relaxed">
          ...
          <code className="text-xs tone-teal">DBMS_DATA_MINING.CREATE_MODEL</code> on 8 transportation service behavioral
          features (value, utilization, engagement, sentiment). Transport Services are assigned using{' '}
          ...
          scores new transportation services automatically.
        </p>
      </div>
      <div className="flex flex-wrap gap-1.5">
...
-- avg_urgentity`} />
      <div className="oml-model-flow">
        ...
        <DiagramBox label="Transport Service Details + Cluster Stats" sub="size · top category · avg probability" color="#4C825C" />
      </div>
    </div>
  );
}
...
function CapacityOraclePanel() {
  return (
    <div className="space-y-4">
      <div>
        <p className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">
          DEMAND_SURGE_MODEL × Capacity - Terminal Access Risk Intelligence
        </p>
        <p className="text-sm text-[var(--color-text)] leading-relaxed">
          Joins <span className="tone-plum font-mono">DEMAND_SURGE_MODEL</span> (Random Forest) predictions with
          live capacity levels across all logistics terminals. Oracle scores each transportation service in real-time using{' '}
          <code className="text-xs tone-plum">PREDICTION_PROBABILITY()</code>, then compares predicted demand
          against available capacity to identify access risk - transportation services where shipper-signal-driven demand will exceed capacity.
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        <FeatureBadge label="capacity × logistics terminals" color="cyan" />
...
ORDER BY oml_surge_probability DESC;`} />
      <div className="oml-model-flow">
        ...
        <div className="text-center text-[10px] text-[var(--color-text-dim)]">↓ JOIN</div>
        <DiagramBox label="capacity × logistics terminals" sub="quantity_on_hand · reorder_point · terminals" color="#437C94" />
        <div className="text-center text-[10px] text-[var(--color-text-dim)]">↓ COMPARE</div>
        <DiagramBox label="Terminal Access Risk: capacity_status + days_of_capacity + service_value_at_risk" sub="NO_CAPACITY · CRITICAL · AT_RISK · ADEQUATE" color="#C74634" />
      </div>
    </div>
  );
}
...
export default function OMLAnalytics() {
  ...
  const shippers  = segData?.shippers    || [];
  const segSummary = segData?.segmentSummary || [];
  const commitmentRiskDist  = segData?.commitmentRiskDistribution || segData?.churnDistribution || [];
  ...
  const filteredShippers = selectedSegment
    ? shippers.filter(c => c.SEGMENT === selectedSegment)
    : shippers;
    ...
    <div className="space-y-6 fade-in">
      ...
      <div>
        <h2 className="text-2xl font-bold flex items-center gap-2">
          <JetGlyph iconClass="oj-fwk-icon-view" className="oml-header-glyph tone-plum" /> Predictive Service Risk & Capacity
        </h2>
        <p className="text-sm text-[var(--color-text-dim)] mt-1">
          Forecast demand surge, assess shipper health, quantify freight value at risk, and recommend capacity action.
        </p>
      </div>
      ...
      <RegisterOraclePanel title="OML Analytics">
        ...
        {activeTab === 'rfm'      && <HealthOraclePanel />}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          label="Services with Demand Surge"
        ...
        <StatCard
          ...
          label="Shippers Segmented"
          value={summaryLoading ? '…' : formatNumber(summary?.TOTAL_CUSTOMERS || summary?.total_shippers || 0)}
          sub="K-Means CLUSTER_ID() + shipper health"
          color="#C74634"
          badge="KM"
        />
        <StatCard
          iconClass="oj-fwk-icon-view"
          label="Freight Value Model Fit"
        ...
        <StatCard
          ...
          sub="Demand · Health · Forecast · K-Means"
          color="#4F7D7B"
          badge="In-DB"
        />
      </div>
      ...
      {/* ══════════════════════════════════════════
          Tab 1 - Transport Demand Predictions
      ══════════════════════════════════════════ */}
      {activeTab === 'demand' && (
        <section
          id="oml-panel-demand"
        ...
        >
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <h3 className="text-base font-bold flex items-center gap-2">
                ...
                Transport Demand Surge Predictions
              </h3>
              <p className="text-xs text-[var(--color-text-dim)] mt-0.5">
                Which services are likely to surge during the Northeast Corridor disruption, with confidence and value exposure.
              </p>
            </div>
            <div className="flex items-center gap-2">
              <JetSelectSingle
          ...
          {demandLoading ? (
            ...
            <p className="text-sm text-[var(--color-text-dim)] py-4 text-center">No transportation services with sufficient shipper-signal activity in this window.</p>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
              {/* Bar chart - predicted demand */}
              <div className="lg:col-span-2">
                <p className="text-[10px] text-[var(--color-text-dim)] uppercase tracking-wider mb-2">
                Top 10 - Predicted Shipment Requests (7-day horizon)
                </p>
                  ...
                  <BarChart data={products.slice(0, 10)} layout="vertical" margin={{ left: 0, right: 16 }}>
                    ...
                    <Tooltip
                      ...
                      formatter={(v, n) => [formatNumber(v), n === 'PREDICTED_DEMAND' ? 'Predicted Shipment Requests' : n]}
                    />
                  ...
                  <div className="oml-demand-reasons">
                    <p className="oml-demand-reasons__title">Top request reasons</p>
                    {products.slice(0, 5).map((p, i) => (
                      <div key={p.PRODUCT_ID || p.PRODUCT_NAME || i} className="oml-demand-reasons__row">
                        <span className="oml-demand-reasons__rank">{i + 1}</span>
                        <div className="min-w-0">
                          <p className="oml-demand-reasons__service">{p.PRODUCT_NAME}</p>
                          <p className="oml-demand-reasons__why">
                            {p.PEAK_MOMENTUM?.replace(/_/g, ' ') || 'active'} signal, +{p.UPLIFT_PCT}% uplift, {p.CONFIDENCE_PCT}% confidence
                          </p>
                        </div>
                      </div>
                    ))}
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="text-[10px] text-[var(--color-text-dim)] uppercase tracking-wider border-b border-[var(--color-border)]">
                      <th className="text-left py-2 px-2">Transport Service</th>
                      ...
                      <th className="text-right py-2 px-2">Freight Value at Risk</th>
          ...
          <div className="rounded-lg p-3 text-[10px] flex flex-wrap gap-x-6 gap-y-1"
            ...
            <span><strong>Features:</strong> 12 - category, value, signal count, sentiment, exposure, shares, urgency, rising signals, units requested, service value</span>
      ...
      {/* ══════════════════════════════════════════
          Tab 2 - Shipper Health Segmentation
      ══════════════════════════════════════════ */}
      {activeTab === 'rfm' && (
        <section
          id="oml-panel-rfm"
        ...
        >
          <div>
              <h3 className="text-base font-bold flex items-center gap-2">
                ...
                Shipper Health Segmentation
              </h3>
            <p className="text-xs text-[var(--color-text-dim)] mt-0.5">
              CUSTOMER_SEGMENT_MODEL (K-Means, 4 clusters) via DBMS_DATA_MINING +{' '}
              <code className="tone-plum">NTILE(4)</code> shipper-health labeling - CLUSTER_ID() scoring
            </p>
          </div>

          {segLoading ? (
            <p className="text-sm text-[var(--color-text-dim)] py-4 text-center">Scoring shippers via CLUSTER_ID(CUSTOMER_SEGMENT_MODEL)...</p>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <div>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <PieChart>
                    ...
                    <Tooltip
                      ...
                      labelStyle={{ color: 'var(--color-text)' }}
                      cursor={false}
                      formatter={(v, n, p) => [`${v} shippers`, p.payload.segment]}
                    />
                  </PieChart>
                </ResponsiveContainer>
                {/* Legend */}
                ...
                {/* Commitment risk bar + segment table */}
              <div className="space-y-4">
                <div>
                  <p className="text-[10px] text-[var(--color-text-dim)] uppercase tracking-wider mb-2">Commitment Risk Distribution</p>
                  <ResponsiveContainer width="100%" height={100}>
                    <BarChart data={commitmentRiskDist} margin={{ top: 0, right: 8, bottom: 0, left: 0 }}>
                      <XAxis dataKey="risk" tick={{ fontSize: 10, fill: '#697778' }} />
                      <YAxis tick={{ fontSize: 9, fill: '#697778' }} width={30} />
                      <Bar dataKey="count" radius={[4, 4, 0, 0]}>
                        {commitmentRiskDist.map((d, i) => (
                    ...
                    {segSummary.map((s, i) => (
                      <div key={i} className="flex items-center justify-between text-[11px]">
                        ...
                        <div className="flex gap-3 text-[var(--color-text-dim)]">
                          <span>{s.count} shippers</span>
                          <span className="tone-sienna">{formatCurrency(s.total_revenue)}</span>
                          <span className="tone-plum">Health {s.avg_rfm}</span>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {/* Shipper table - filtered by selected segment */}
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[10px] text-[var(--color-text-dim)] uppercase tracking-wider">
                    {selectedSegment ? `${selectedSegment} shippers` : 'Top shippers by account health'}
                  </p>
                  {selectedSegment && (
                    <JetButton
                      label="Clear"
                ...
                <div className="overflow-y-auto max-h-[240px] space-y-1">
                  {filteredShippers.slice(0, 40).map((c, i) => (
          ...
          <div className="rounded-lg p-3 text-[10px] flex flex-wrap gap-x-6 gap-y-1"
            ...
            <span><strong>Model:</strong> Shipper health via Oracle NTILE(4) - ISO SQL:2003 Window Functions</span>
            <span><strong>Segments:</strong> Strategic Account · Reliable Shipper · New Shipper · SLA Risk · Dormant Shipper · High-Value Shipper · Growth Lane · Developing Account</span>
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <h3 className="text-base font-bold flex items-center gap-2">
                ...
                Freight Value Forecast - Oracle Linear Regression
              </h3>
              <p className="text-xs text-[var(--color-text-dim)] mt-0.5">
              ...
              {model && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    ...
                    { label: 'Mean Daily Freight Value', value: formatCurrency(model.mean_daily_revenue), color: '#C74634' },
              ...
              <ResponsiveContainer width="100%" height={280}>
                <AreaChart data={chartData} margin={{ top: 8, right: 16, bottom: 0, left: 8 }}>
                  ...
                  <Area type="monotone" dataKey="actual" stroke="#4C825C" fill="url(#actualGrad)"
                    strokeWidth={2} dot={false} name="Actual Freight Value" connectNulls={false} />
      ...
      {/* ══════════════════════════════════════════
          Tab 4 - Service Similarity Clusters
      ══════════════════════════════════════════ */}
      {activeTab === 'clusters' && (
        <section
        ...
        >
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <h3 className="text-base font-bold flex items-center gap-2">
                ...
                Service Similarity Clusters
              </h3>
              <p className="text-xs text-[var(--color-text-dim)] mt-0.5">
                  Transportation services grouped by lane, mode, and demand-signal similarity so operators can spot related risk cohorts.
              </p>
            </div>
            <div className="flex items-center gap-2">
          ...
          {clusterLoading ? (
            <div className="py-8 text-center">
                <JetProgressCircle className="oml-loading-progress" ariaLabel="Grouping related transportation services" />
                <p className="text-sm text-[var(--color-text-dim)]">Grouping related transportation services (K={clusterK})...</p>
            </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                    ...
                    { label: 'Services Compared', value: clusterData.total_products, color: '#C74634' },
                    { label: 'Similarity Basis', value: 'Lane + demand', color: '#AA643B' },
                    { label: 'Operational Use', value: 'Risk cohorts', color: '#4C825C' },
                ].map((m, i) => (
                  <div key={i} className="rounded-lg p-3 text-center"
                    style={{ background: `${m.color}11`, border: `1px solid ${m.color}33` }}>
                    <p className="text-[10px] text-[var(--color-text-dim)] mb-1">{m.label}</p>
                  ...
                  {clusterData.clusters.map((cl, i) => (
                    ...
                    >
                      ...
                      <div className="absolute -top-8 bg-[var(--color-surface)] border border-[var(--color-border)] rounded px-2 py-1 text-[9px] whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity z-10">
                        Cluster {cl.cluster_id}: {cl.size} transportation services · {cl.top_category}
                      </div>
                    </div>
                  ))}
                </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-[var(--color-text-dim)]">
                              {cl.size} transportation services · Avg similarity: <span className="font-mono" style={{ color }}>{(cl.avg_similarity * 100).toFixed(1)}%</span>
                      ...
                      <div className="px-4 py-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5">
                        ...
                        {cl.products.length > 12 && (
                          <div className="text-[10px] text-[var(--color-text-dim)] px-2 py-1">
                            +{cl.products.length - 12} more transportation services
                          </div>
                        )}
                      </div>
                    </div>
              ...
              <div className="rounded-lg p-3 text-[10px] flex flex-wrap gap-x-6 gap-y-1"
                ...
                <span><strong>Model:</strong> Service similarity clustering via VECTOR_DISTANCE centroid assignment</span>
                ...
                <span><strong>K:</strong> {clusterData.k} clusters · {clusterData.total_products} transportation services</span>
              </div>
            </>
          )}
        </section>
        ...
        >
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-xs text-[var(--color-text-dim)] mt-0.5">
                DEMAND_SURGE_MODEL predictions × live capacity - identifies terminal access risk from shipper-signal-driven demand surges
              </p>
            </div>
            <JetButton
            ...
            <>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
                <div className="rounded-lg p-3 text-center" style={{ background: '#C7463411', border: '1px solid #C7463433' }}>
                  <p className="text-[10px] text-[var(--color-text-dim)] mb-1">Critical Capacity</p>
                ...
                <div className="rounded-lg p-3 text-center" style={{ background: '#437C9411', border: '1px solid #437C9433' }}>
                  <p className="text-[10px] text-[var(--color-text-dim)] mb-1">SLA Risk (demand {'>'} capacity)</p>
                ...
                <div className="rounded-lg p-3 text-center" style={{ background: '#79608711', border: '1px solid #79608733' }}>
                  <p className="text-[10px] text-[var(--color-text-dim)] mb-1">Freight Value at Risk</p>
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <PieChart>
                      ...
                      <Tooltip
                        ...
                        labelStyle={{ color: 'var(--color-text)' }}
                        cursor={false}
                        formatter={(v, n, p) => [`${v} service-terminal pairs`, capacityStatusLabel(p.payload.status)]}
                      />
                    </PieChart>
                  </ResponsiveContainer>
                  <div className="flex flex-wrap gap-1.5 justify-center">
              ...
              <div className="rounded-lg p-3 text-[10px] flex flex-wrap gap-x-6 gap-y-1"
                ...
                <span><strong>Data:</strong> demand_forecasts (daily OML predictions) × capacity × logistics terminals</span>
                <span><strong>Engine:</strong> Oracle DBMS_DATA_MINING - shipper-signal demand surge to terminal access risk assessment</span>
              </div>
            </>
          )}
        </section>
