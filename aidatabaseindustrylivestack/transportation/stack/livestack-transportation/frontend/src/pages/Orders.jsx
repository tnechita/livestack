...
import { Filter, ChevronRight, ChevronDown, Loader2, Check, MapPin, Package, Truck, Navigation, Shield, Eye } from 'lucide-react';
...
const SHIP_STATUS_STEPS = [
  { key: 'preparing', label: 'Tendered', icon: Package },
  { key: 'picked', label: 'Assigned', icon: Package },
  { key: 'packed', label: 'Loaded', icon: Package },
          ...
          <div className="orders-detail-tabbar">
            <div className="orders-detail-tabset">
              ...
              <span className="text-[10px] text-[var(--color-text-dim)] ml-3 hidden sm:inline">
                Same shipment record - three views
              </span>
            </div>
            <JetButton
              label="Close"
          ...
          {view === 'relational' && (
            <div className="p-4 space-y-3">
              {loadingDetail ? (
                ...
                <p className="text-sm text-[var(--color-text-dim)] text-center py-4">Transport order not found</p>
              ) : (
                <>
                  ...
                  {/* Shipment Order summary */}
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      {[
                        { label: 'Shipper', value: `${order.FIRST_NAME} ${order.LAST_NAME}` },
                        { label: 'Location', value: `${order.CITY}, ${order.STATE_PROVINCE}` },
                        { label: 'Freight Value', value: formatCurrency(order.ORDER_TOTAL) },
                        { label: 'Transport Cost', value: formatCurrency(order.SHIPPING_COST) },
                      ].map(s => (
                    ...
                    {order.ORDER_STATUS === 'cancelled' && order.CANCELLATION_REASON && (
                      <div className="rounded-lg p-3 text-xs leading-relaxed" style={{ background: 'rgba(199,70,52,0.06)', border: '1px solid rgba(199,70,52,0.22)' }}>
                        <span className="font-semibold tone-red">Cancellation reason: </span>
                        <span className="text-[var(--color-text)]">{order.CANCELLATION_REASON}</span>
                      </div>
                    )}
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="text-[10px] text-[var(--color-text-dim)] uppercase tracking-wider border-b border-[var(--color-border)]">
                        <th className="text-left py-1.5 px-2">Leg #</th>
                        <th className="text-left py-1.5 px-2">Transport Service</th>
                        <th className="text-left py-1.5 px-2">Service Line</th>
                        <th className="text-left py-1.5 px-2">Category</th>
                        <th className="text-right py-1.5 px-2">Loads</th>
                        <th className="text-right py-1.5 px-2">Contract Rate</th>
                        <th className="text-right py-1.5 px-2">Leg Value</th>
                      </tr>
                    </thead>
                    <tbody>
                      {items.map(item => (
              ...
              {loadingDuality ? (
                ...
                <>
                  ...
                  <div className="rounded-lg p-3 text-xs leading-relaxed" style={{ background: 'rgba(170,100,59,0.06)', border: '1px dashed rgba(170,100,59,0.3)' }}>
                    ...
                    <span className="text-[var(--color-text-dim)]"> - This is the exact same shipment order data from the relational tab, but accessed through
                  ...
                  <div className="rounded-lg overflow-hidden" style={{ background: 'var(--color-surface-muted)', border: '1px solid rgba(170,100,59,0.25)' }}>
                    <div className="px-3 py-1.5 flex items-center justify-between" style={{ background: 'rgba(170,100,59,0.08)', borderBottom: '1px solid rgba(170,100,59,0.2)' }}>
                      ...
                      <span className="text-[10px] text-[var(--color-text-dim)] font-mono">
                        {duality.document.items?.length || 0} nested service legs
                      </span>
                    </div>
                  ...
                  <>
                    ...
                    <div className="orders-route-map">
                      ...
                      >
                        ...
                        {/* Logistics Terminal center marker */}
                        <CircleMarker center={from} radius={8} fillColor="#437C94" fillOpacity={0.9} color="#fff" weight={2}>
                          <Tooltip permanent direction="top" offset={[0, -10]}
                            ...
                            <div className="route-map-tooltip__content">
                              ...
                              <span>{order.CENTER_NAME || 'Logistics Terminal'}</span>
                            </div>
                          </Tooltip>
                        </CircleMarker>
                        ...
                        <CircleMarker center={to} radius={8} fillColor="#4C825C" fillOpacity={0.9} color="#fff" weight={2}>
                          <Tooltip permanent direction="top" offset={[0, -10]}
                            className="route-map-tooltip">
                            <div className="route-map-tooltip__content">
                              <span className="route-map-tooltip__label">Shipper</span>
                    ...
                    {/* Shipment route info cards */}
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      {[
                        { label: 'Distance', value: distanceMiles ? `${Math.round(distanceMiles).toLocaleString()} mi` : '-', color: '#4C825C' },
                        { label: 'Est. Transit', value: estHours ? `${estHours} hrs` : '-', color: '#AA643B' },
                        { label: 'Transport Cost', value: shipment?.SHIP_COST ? formatCurrency(shipment.SHIP_COST) : '-', color: '#437C94' },
...
-- SDO_GCDR.ELOC_DRIVE_TIME_POLYGON for isochrone zones` : `-- Distance between logistics terminal and shipper
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-[var(--color-text)] leading-relaxed">
              Shipment orders are stored in a classic <span className="tone-ocean font-mono">relational schema</span> - ACID transactions, foreign keys, referential integrity.
          ...
          <div>
            <p className="text-[10px] font-semibold text-[var(--color-text)] uppercase tracking-wider mb-2">
              ORDERS_DV - Shipment Orders + Service Legs as JSON
            </p>
            <p className="text-xs text-[var(--color-text-dim)] mb-2 leading-relaxed">
              REST-style JSON access to shipment orders with nested service legs. Read-write: inserts through the view update both <span className="font-mono text-[var(--color-text)]">orders</span> and{' '}
...
FROM orders o WITH UPDATE;`} />
          ...
          <div>
            <p className="text-[10px] font-semibold text-[var(--color-text)] uppercase tracking-wider mb-2">
              PRODUCTS_INVENTORY_DV - Transport Services + Terminal Capacity as JSON
            </p>
            <p className="text-xs text-[var(--color-text-dim)] mb-2 leading-relaxed">
              Transport services with nested terminal capacity across all logistics terminals. One document, two tables.
            </p>
...
WHERE  JSON_VALUE(DATA, '$._id' RETURNING NUMBER) = :id;
  -- Returns: {"_id":1, "status":"cancelled", "cancellationReason":"Carrier no-show", "serviceLegs":[...]}`} />
          </div>
          ...
          <div className="rounded-lg p-3 space-y-2" style={{ background: 'var(--color-surface-muted)', border: '1px solid var(--color-border)' }}>
            ...
            <DiagramBox label="orders + order_items" sub="Normalized shipment tables - ACID, FK constraints, indexes" color="#437C94" />
            <div className="text-center text-[10px] text-[var(--color-text)]">↕ CREATE JSON RELATIONAL DUALITY VIEW</div>
            <DiagramBox label="ORDERS_DV" sub="JSON document: {_id, status, serviceLegs: [...]} - WITH UPDATE" color="#AA643B" />
            <div className="text-center text-[10px] text-[var(--color-text)]">↕</div>
            <DiagramBox label="products + inventory" sub="Transport service catalog + terminal capacity levels" color="#437C94" />
            <div className="text-center text-[10px] text-[var(--color-text)]">↕ CREATE JSON RELATIONAL DUALITY VIEW</div>
            <DiagramBox label="PRODUCTS_INVENTORY_DV" sub="JSON document: {sku, serviceName, terminalCapacity: [...]} - WITH UPDATE" color="#AA643B" />
          </div>
          ...
          {/* VPD on shipment orders */}
          <div>
            <p className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">
              Virtual Private Database (VPD) on Shipment Orders
            </p>
            <p className="text-sm text-[var(--color-text)] leading-relaxed mb-2">
              ...
              <code className="text-xs tone-teal mx-1">ORDERS</code> table restricts dispatch managers
              to shipment orders routed through their regional terminals. The policy function queries{' '}
          ...
          <div className="flex flex-wrap gap-1.5">
            <FeatureBadge label="DBMS_RLS (Shipment Orders)" color="red" />
...
BEGIN
    ...
    -- Logistics terminal mgr: shipment orders for their region's terminals
  ...
  statement_types => 'SELECT');`} />
      ...
      <div>
        <h2 className="text-2xl font-bold flex items-center gap-2">
          <Truck className="tone-ocean" /> Shipment Orders & Exceptions
        </h2>
        <p className="text-sm text-[var(--color-text-dim)] mt-1">
          Click any shipment order to inspect the operational record, JSON payload, and route execution view.
        </p>
      </div>
          ...
          >
            ...
            <div className="flex items-center gap-2 flex-wrap flex-1">
              ...
              <span className="text-[var(--color-text-dim)] text-xs">
                {isFM
                  ? `Filtered to ${currentUser.REGION} - ${(orders || []).length} shipment orders visible`
                  : `${(orders || []).length} shipment orders visible`
                }
              </span>
            </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-[11px] text-[var(--color-text-dim)] uppercase tracking-wider border-b border-[var(--color-border)] bg-[var(--color-surface)]/50">
                <th className="text-left py-3 px-4">Shipment Order #</th>
                <th className="text-left py-3 px-4">Shipper</th>
                <th className="text-left py-3 px-4">Location</th>
                  <th className="text-center py-3 px-4">Status</th>
                  <th className="text-left py-3 px-4">Reason</th>
                  <th className="text-right py-3 px-4">Service Legs</th>
                <th className="text-right py-3 px-4">Freight Value</th>
                <th className="text-center py-3 px-4">Operational Signal</th>
                <th className="text-left py-3 px-4">Logistics Terminal</th>
                <th className="text-left py-3 px-4">Created</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                  <tr><td colSpan={10} className="py-8 text-center text-[var(--color-text-dim)]">Loading shipment orders...</td></tr>
                ) : (orders || []).length === 0 ? (
                  <tr><td colSpan={10} className="py-8 text-center text-[var(--color-text-dim)]">No shipment orders found</td></tr>
              ) : (
                (orders || []).map(o => {
                    ...
                    <tr key={o.ORDER_ID}
                      ...
                      <td className="py-3 px-4 text-[var(--color-text-dim)]">{o.CUSTOMER_CITY}, {o.CUSTOMER_STATE}</td>
                        ...
                        <td className="py-3 px-4 text-xs">
                          {o.ORDER_STATUS === 'cancelled' && o.CANCELLATION_REASON ? (
                            <span className="font-medium tone-red">{o.CANCELLATION_REASON}</span>
                          ) : (
                      ...
                      <td className="py-3 px-4 text-center">
                        {o.SOCIAL_DRIVEN ? (
                          <span className="tone-rose text-xs font-semibold">Operational Signal</span>
                        ) : (
