...
/* ─── Transport Service Detail Modal ─────────────────────────────────────────────── */
      ...
      >
        ...
        <div className="flex items-start justify-between p-5 border-b border-[var(--color-border)]">
          {loading ? (
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {product.PEAK_MOMENTUM && (
                  <span className={`momentum-badge momentum-${product.PEAK_MOMENTUM}`}>
                    {product.PEAK_MOMENTUM === 'mega_urgent' ? '🔥 MEGA' : product.PEAK_MOMENTUM?.replace('_', ' ')}
                  </span>
                )}
              </div>
            ...
            <p className="text-sm tone-red">{error || 'Failed to load transport service'}</p>
          )}
        ...
        {!loading && product && (
          <div className="flex items-center gap-1 px-5 pt-3 pb-0">
            ...
            <button onClick={() => setTab('json')}
              ...
              Same service data - two interfaces
            </span>
          </div>
        )}
          ...
          <div className="p-5 space-y-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="glass-card p-3 text-center" style={{ background: 'rgba(170,100,59,0.05)', borderColor: 'rgba(170,100,59,0.2)' }}>
                ...
                <p className="text-[10px] text-[var(--color-text-dim)]">Committed Capacity</p>
              </div>
              <div className="glass-card p-3 text-center" style={{ background: 'rgba(67,124,148,0.05)', borderColor: 'rgba(67,124,148,0.2)' }}>
                ...
                <p className="text-[10px] text-[var(--color-text-dim)]">Operational Signals</p>
              </div>
            </div>
            ...
            {inventory.length > 0 && (
              <div>
                <h4 className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2 flex items-center gap-1.5">
                  <MapPin size={12} /> Capacity by Logistics Terminal
                </h4>
                              ...
                              <span className="px-1.5 py-0.5 rounded text-[9px] font-medium"
                                style={{
                                  background: inv.CENTER_TYPE === 'distribution' ? 'rgba(67,124,148,0.15)' :
                                              inv.CENTER_TYPE === 'terminal' ? 'rgba(76,130,92,0.15)' : 'rgba(170,100,59,0.15)',
                                  color: inv.CENTER_TYPE === 'distribution' ? '#437C94' :
                                         inv.CENTER_TYPE === 'terminal' ? '#4C825C' : '#AA643B',
                                }}>
                                {inv.CENTER_TYPE}
                              </span>
                            </td>
            ...
            {mentions.length > 0 && (
              <div>
                <h4 className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2 flex items-center gap-1.5">
                  <MessageSquare size={12} /> Recent Operational Signals
                </h4>
            ...
            {inventory.length === 0 && mentions.length === 0 && (
              <p className="text-sm text-[var(--color-text-dim)] text-center py-4">No detailed data available for this transportation service.</p>
            )}
          </div>
        )}
            ...
            {loadingDuality ? (
              ...
              <>
                ...
                <div className="rounded-lg p-3 text-xs leading-relaxed" style={{ background: 'rgba(170,100,59,0.06)', border: '1px dashed rgba(170,100,59,0.3)' }}>
                  <span className="text-[#AA643B] font-semibold">Transport Service + Capacity as JSON Document</span>
                  <span className="text-[var(--color-text-dim)]"> - The same transportation service and terminal capacity data from the Details tab, exposed as a single nested JSON document.
                  The duality view joins <span className="text-[#437C94] font-mono">products</span> (transportation services) and <span className="text-[#437C94] font-mono">inventory</span> (terminal capacity) tables
                  into one document with a nested capacity array.</span>
                </div>
                ...
                <div className="dashboard-duality-json-panel">
                  <div className="dashboard-duality-json-panel__header">
                    ...
                    <span className="text-[10px] text-[var(--color-text-dim)] font-mono">
                      {duality.document.inventory?.length || 0} terminal capacity locations
                    </span>
                  </div>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-[11px] text-[var(--color-text-dim)] uppercase tracking-wider border-b border-[var(--color-border)]">
            <th className="text-left py-2 px-3">Transport Service</th>
              ...
              >
                ...
                <td className="py-2.5 px-3 text-center">
                  <span className={`momentum-badge momentum-${p.PEAK_MOMENTUM}`}>
                    {p.PEAK_MOMENTUM === 'mega_urgent' ? '🔥 MEGA' : p.PEAK_MOMENTUM?.replace('_', ' ')}
                  </span>
                </td>
...
const MOMENTUM_FILTERS = ['', 'mega_urgent', 'urgent', 'rising'];
const MOMENTUM_LABELS  = { '': 'All', mega_urgent: '🔥 Mega', urgent: 'Urgent', rising: 'Rising' };
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6 fade-in">
      <TransportationStoryPanel scene="dashboard" />
...
SELECT
  ...
  COUNT(sp.post_id) FILTER (
    WHERE sp.momentum_flag = 'urgent')   AS urgent_posts,
...
FROM   orders o, social_posts sp,
       agent_actions aa, shipments s;`} />
          <SqlBlock code={`-- Transport service search: Oracle UPPER() case-insensitive LIKE
SELECT p.product_name, b.brand_name,
       COUNT(DISTINCT ppm.post_id) AS mention_count,
       ROUND(AVG(sp.urgentity_score), 2) AS avg_urgentity
FROM products p
...
ORDER BY avg_urgentity DESC;`} />
          <div>
            ...
            <div className="grid grid-cols-3 gap-1.5">
              <DiagramBox label="JSON Docs" sub="operational signal event stream" color="#AA643B" />
              ...
              <DiagramBox label="Relational" sub="orders shippers" color="#437C94" />
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">Logistics Control Tower</h2>
          <p className="text-sm text-[var(--color-text-dim)] mt-1">
            Northeast Corridor disruption, terminal pressure, shipment exposure, and recovery decisions
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        <StatCard iconClass="oj-fwk-icon-tree-document" label="Shipment Orders" value={formatNumber(s.ORDERS_TOTAL)} subValue={`${formatNumber(s.ORDERS_30D)} last 30d`} color="#437C94" />
        <StatCard iconClass="oj-fwk-icon-view" label="Freight Value at Risk" value={formatCurrency(s.REVENUE_TOTAL)} subValue={`${formatCurrency(s.REVENUE_30D)} last 30d`} color="#4C825C" />
        <StatCard iconClass="oj-fwk-icon-message-warning" label="Urgent Signals" value={formatNumber(s.VIRAL_POSTS)} subValue={`${formatNumber(s.RISING_POSTS)} rising`} color="#C74634" />
        <StatCard iconClass="oj-fwk-icon-sortrelevancehigh" label="Services at Surge Risk" value={formatNumber(s.TRENDING_PRODUCTS)} subValue={`${formatNumber(s.POSTS_TOTAL)} total signals`} color="#AA643B" />
        <StatCard iconClass="oj-fwk-icon-users" label="Agent Actions" value={formatNumber(s.AGENT_ACTIONS_TOTAL)} subValue={`${formatNumber(s.SHIPMENTS_IN_TRANSIT)} in transit`} color="#796087" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="glass-card p-5 lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            ...
            <p className="text-xs text-[var(--color-text-dim)] mb-3">
              Signal Velocity shows how quickly new disruption, demand, and service-risk signals are building over the selected time window.
            </p>
            ...
            <div className="flex items-center justify-center" style={{ height: 240 }}>
              <div className="text-center space-y-2">
                ...
                <p className="text-sm text-[var(--color-text-dim)]">No operational signals during this time period</p>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={velocity || []}>
              ...
              />
              <Area type="monotone" dataKey="TOTAL_LIKES" stroke="#AA643B" fill="url(#gradLikes)" strokeWidth={2} name="Signal Exposure" />
              <Area type="monotone" dataKey="VIRAL_COUNT" stroke="#C74634" fill="url(#gradUrgent)" strokeWidth={2} name="Urgent Signals" />
            </AreaChart>
          </ResponsiveContainer>
          )}
        </div>
        ...
        <div className="glass-card p-5">
            <h3 className="text-sm font-semibold mb-4 flex items-center gap-2">
              ...
              Freight Value by Service Category
            </h3>
            <p className="text-xs text-[var(--color-text-dim)] mb-3">
              Categories show where freight value exposure is concentrated across related transportation services.
            </p>
            <ResponsiveContainer width="100%" height={240}>
            <PieChart>
              <Pie
      ...
      <div className="glass-card p-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-4">
          ...
          <div className="relative flex-1 min-w-0">
            ...
            <input
              ...
              placeholder="Search transportation services, lanes, or programs..."
        ...
        {loadingTrending ? (
          <p className="text-sm text-[var(--color-text-dim)]">Loading services at surge risk...</p>
        ) : (
          <TrendingTable
            products={trending}
        ...
        {!loadingTrending && trending?.length === 0 && (
          <p className="text-sm text-[var(--color-text-dim)] text-center py-6">No transportation services match your search.</p>
        )}

        <p className="text-[10px] text-[var(--color-text-dim)] mt-3">
          Click any row to view terminal capacity and operational signal evidence
        </p>
      </div>
      ...
      <div className="glass-card p-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex gap-2 flex-wrap">
            {[
              { label: 'Relational', desc: 'Shipment Orders & Capacity', color: '#437C94' },
              { label: 'JSON', desc: 'Signal Payloads', color: '#4C825C' },
              { label: 'Graph', desc: 'Risk Network', color: '#AA643B' },
                { label: 'Similarity', desc: 'Related Signal Matching', color: '#796087' },
              { label: 'Spatial', desc: 'Dispatch Routing', color: '#4F7D7B' },
