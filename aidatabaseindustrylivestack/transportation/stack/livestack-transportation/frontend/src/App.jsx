...
const CUSTOMER_STORAGE_KEY = 'transportationLivestack.customerName';
const CUSTOMER_ALIASES = [
  'Seer Transport',
  'Transportation Fleet Logistics',
  'Transportation Demo Data',
  'Seer Equity Bank',
  'Seer Equity',
  'Seer Bank',
];
...
const NAV_ITEMS = [
  ...
  { id: 'dashboard', label: 'Fleet Risk & Operations Dashboard', iconClass: 'oj-fwk-icon oj-fwk-icon-grid', featureTags: ['In-Memory', 'Native JSON'] },
  { id: 'social', label: 'Disruption & Demand Signals', iconClass: 'oj-fwk-icon oj-fwk-icon-sortrelevancehigh', featureTags: ['Similarity Search', 'Row-Level Security'] },
  { id: 'graph', label: 'Transportation Risk Network', iconClass: 'oj-fwk-icon oj-fwk-icon-node-expand', featureTags: ['Property Graph', 'SQL/PGQ'] },
  { id: 'fulfillment', label: 'Network Capacity & Rerouting', iconClass: 'oj-fwk-icon oj-fwk-icon-calendar-clock', featureTags: ['Oracle Spatial', 'VPD'] },
  { id: 'orders', label: 'Shipment Orders & Exceptions', iconClass: 'oj-fwk-icon oj-fwk-icon-tree-document', featureTags: ['JSON Duality', 'VPD'] },
  { id: 'oml', label: 'Predictive Service Risk & Capacity', iconClass: 'oj-fwk-icon oj-fwk-icon-view', featureTags: ['Predictive Models', 'Service Clustering'] },
  { id: 'askdata', label: 'Ask Seer Transport Data', iconClass: 'oj-fwk-icon oj-fwk-icon-magnifier', featureTags: ['Natural Language SQL', 'Live Schema'] },
  { id: 'agents', label: 'Operations Agent Console', iconClass: 'oj-fwk-icon oj-fwk-icon-users', featureTags: ['PL/SQL Tools', 'JSON Audit'] },
];
...
function replaceCustomerTerms(value, profile) {
  ...
  let next = value.replace(/\bseer-transport\.example\b/gi, profile.customerDomain);
  next = next.replace(/\bseer-equity\.example\b/gi, profile.customerDomain);
...
function hasReplaceableCustomerTerms(value) {
  return typeof value === 'string' && (
    /\bseer-(transport|equity)\.example\b/i.test(value) ||
    /\bSeer(?: Transport| Equity Bank| Equity| Bank)?\b/.test(value) ||
    /\bTransportation(?: Fleet Logistics| Demo Data)?\b/.test(value)
  );
}
...
function OracleBrand({ profile }) {
  const title = replaceCustomerTerms(`${CUSTOMER_NAME} Fleet Logistics LiveStack`, profile);
  return (
    <button
      type="button"
    ...
    <div className="customer-name-tool">
      ...
      {isOpen && (
        <form className="customer-name-tool__panel" onSubmit={submit}>
          ...
          <p className="customer-name-tool__hint">Replaces Seer Transport, Seer, and inherited Seer Equity labels on visible screens.</p>
        </form>
      )}
    </div>
  );
