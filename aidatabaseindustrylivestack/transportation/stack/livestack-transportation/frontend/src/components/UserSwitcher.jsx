...
export default function UserSwitcher() {
  ...
  if (!currentUser) {
    return (
      <p className="text-[10px] leading-4 p-2 border" role="status" style={{ borderColor: 'var(--color-border)', color: 'var(--color-text-dim)' }}>
        {identityError || 'No governed identity is available.'}
      </p>
    );
  }
    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        ...
        >
            ...
            {users.map((user) => {
              ...
              return (
                <button
                  ...
                  disabled={switchingIdentity}
                  onClick={async () => {
                    const switched = await switchUser(user.USERNAME);
          ...
          >
            {switchingIdentity
              ? 'Establishing the signed governed session...'
              : 'Switching clears the prior view before the governed scope reloads.'}
          </div>
        </div>
      )}
    </div>
